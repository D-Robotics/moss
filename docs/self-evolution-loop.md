...
- [~] T3.1 层 1 契约库:[✅ AcceptSpec + 契约加载器 + ContractRegistry(按 tool 反查)+ 谓词执行器 + hook 接契约产 L1,3/20 示范契约生效(rdk-board-knowledge/rdk-device/rdk-ros)] / [待:其余 17 skill 填契约 + PlanStep.expectedAccept 接线(解 A)+ 多契约覆盖同 tool 的优先级(TODO:更窄 expectedTools 优先)]
...
- [x] **U5** ✅ 已设计成可执行用例(D5/D6 可信根边界验收测试,见 §8 附录 B):
