...
由 地瓜机器人 (D-Robotics) 打造。项目是 **Moss**；包是 **`@rdk-moss/agent`**；CLI 是 **`moss`**（`moss` 为兼容别名）。
...
`/connect <ip>` 先提示输入 SSH 账号和隐藏显示的密码，再把活动会话搬到 RDK 板上（板端模式：设备 + ROS2 工具）；`/disconnect` 恢复本地工具。配置 `MOSS_DEVICE_HOST` 后可直接输入 `/connect`，密钥认证需显式传入 `--key`。在 RDK 板上，用 [device-knowledge](https://github.com/D-Robotics/device-knowledge) 知识包把整套栈教给 Moss——见 [Skills 用户指南](../../docs/user-guide/08-skills.md)。
...
支持：`deepseek`、`qwen`、`openai`、`anthropic`、`openai-compatible`。设置只存在 moss 配置里——`OPENAI_API_KEY` / `MOSS_PROVIDER` 等环境变量被刻意忽略，这样为别的工具导出的 Key 不会悄悄切换你的服务商（`moss doctor` 会列出残留）。脚本/CI 用 `moss --config-file /path/to/config.json`。优先级：CLI 参数 / `-c key=value` > 项目 `.moss/config.json` > `moss config` > 内置网关。
