...
- Multiple `MossAgent` instances in the same process share state — registrations from one agent leak into another.
...
Instance-scoped APIs give each `MossAgent` its own `KnowledgeRegistry` and `PlatformExtensionRegistry`, providing true isolation.

As of the current unreleased version, `agent.extensions.*` is backed by a private
`PlatformExtensionRegistry` per `MossAgent`. Deprecated extension free functions
...
**Behavioral difference:** The deprecated function registers to a process-scoped singleton and also pushes to a pending queue that is drained into new `MossAgent` instances at construction time. The instance method registers directly to the agent's own `KnowledgeRegistry` — no bridging, no cross-agent leakage.
