...
const event = {
  ...
  experienceIds: ['exp-fail', 'exp-pass'],
  previousFailureId: 'failure-2',
...
assert.equal(
  validateRecoveryRecipe(recipe, 'capture a stable frame and convert it'),
  'quality_passed'
);
...
assert.equal(
  validateRecoveryRecipe(collisionRecipe, 'capture a stable frame and convert it'),
  'quality_passed'
);
...
console.log(
  'actionable recovery recipe: trusted compilation, sanitization, quality and shadow isolation ok'
);
