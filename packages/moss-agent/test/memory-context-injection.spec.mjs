#!/usr/bin/env node
/**
 * MossAgent memoryContextProvider injection.
 *
 ...
 *   node packages/moss-agent/test/memory-context-injection.spec.mjs
 */
...
// 3. no provider → no memory block injected (control; guidance text is in stable, not dynamic)
{
  ...
  assert.ok(!streamRequests[0].systemPromptParts.dynamic.includes('<moss_memory>'), 'no memory block without a provider');
}
...
// 4. empty digest → nothing injected, run still completes
{
  ...
  assert.ok(!streamRequests[0].systemPromptParts.dynamic.includes('<moss_memory>'), 'empty digest injects nothing');
}
...
// 5. a throwing provider is non-fatal — the run still reaches the model
{
  ...
  assert.ok(!streamRequests[0].systemPromptParts.dynamic.includes('<moss_memory>'), 'no block when the provider throws');
}
