 ...
 * had no identity in its system prompt. The fix passes MOSS_CLI_IDENTITY as the
 ...
 *   node packages/moss-agent/test/cli-identity.spec.mjs
 */
...
import { MOSS_CLI_IDENTITY, buildMossCliIdentity } from '../dist/cli/identity.js';
...
test('MOSS_CLI_IDENTITY keeps the Moss persona but is honest about the model', () => {
  assert.match(MOSS_CLI_IDENTITY, /\bMoss\b/);
  assert.match(MOSS_CLI_IDENTITY, /D-Robotics/);
  assert.match(MOSS_CLI_IDENTITY, /地瓜机器人/);
  assert.match(MOSS_CLI_IDENTITY, UNIQUE_CLAUSE);
  assert.match(MOSS_CLI_IDENTITY, MODEL_HONESTY, 'identity must allow honest model disclosure');
  // bilingual: includes the Chinese identity too
  assert.match(MOSS_CLI_IDENTITY, /地瓜机器人（D-Robotics）研发的 Agent/);
});

test('buildMossCliIdentity names the actual model (honest, not substituted with "Moss")', () => {
  const withModel = buildMossCliIdentity({ model: 'deepseek-v4-pro' });
...
test('CLI identity lands in buildSystemPrompt when passed as baseSystemPrompt', () => {
  const frame = newAgent({ baseSystemPrompt: MOSS_CLI_IDENTITY }).buildSystemPrompt();
...
test('CLI process sends the Moss identity in the provider system prompt', async () => {
  ...
  try {
    ...
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-identity-'));
    const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-identity-config-'));
    fs.writeFileSync(path.join(configDir, 'community-auth.json'), JSON.stringify({
      schema: 'moss_community_auth.v1',
    ...
    // API key must come from the config file, not MOSS_API_KEY.
    ...
    assert.equal(
      result.status,
      0,
      `moss identity smoke should exit cleanly\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`,
    );
