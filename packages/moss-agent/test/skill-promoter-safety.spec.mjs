...
try {
  const preserved = path.join(workspace, '.moss', 'skills', 'operator-skill', 'references', 'keep.txt');
  ...
  await fs.writeFile(path.join(candidateDir, 'candidate.json'), JSON.stringify({
    candidateId: '!!!', sourceKind: 'conversation', createdAt: Date.now(),
    sourceSessionKey: 'safety-test', turnHash: 'turn', gate: 'strict',
    toolCalls: [{ name: 'read_file', input: {}, failed: false }], toolNames: ['read_file'],
    userMessage: 'read a file', assistantText: 'done',
