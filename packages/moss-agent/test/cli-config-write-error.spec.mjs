 ...
 *   node packages/moss-agent/test/cli-config-write-error.spec.mjs
 */
...
{
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cfg-write-'));
  try {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [distCli, 'config', 'set', 'model', 'test-model'], {
      ...
      env: {
        ...
        MOSS_CONFIG_DIR: path.join(fileAsParent, 'sub'),
