...
const registry = ContractRegistry.fromSkills(
  new SkillRegistry({ workspaceDir: process.cwd() }).list()
);
for (const skill of ['rdk-capture-photo', 'rdk-isp-tuning', 'rdk-hardware', 'rdk-command-manual']) {
  assert.ok(
    registry.findBySkill(skill),
    `${skill} contract must load through the production registry`
  );
}
assert.equal(
  registry.findByTool('device_exec', { command: 'ffmpeg -i frame.yuv /tmp/photo.jpg -y' })
    ?.skillName,
  'rdk-capture-photo'
);
...
assert.equal(
  (await evaluatePredicate({ name: 'file_nonempty', params: { path: jpegPath } }, predicateInput))
    .verdict,
  'pass'
);
assert.equal(
  (await evaluatePredicate({ name: 'image_decodable', params: { path: jpegPath } }, predicateInput))
    .verdict,
  'pass'
);
...
const appendExperience = (runId, evidenceId, verdict) =>
  experienceLog.append({
    ...
    attemptId: `${runId}:${evidenceId}`,
    contractSkill: 'skill-a',
    contractVersion: '1',
    environmentFingerprint: 'local-env',
    executionDomain: 'local',
    realEvidenceEligible: false,
    tool: 'exec',
    input: {},
...
const simulatedRecovery = {
  ...
  environmentFingerprint: 'sim-env',
  environmentIdentityVersion: 1,
  environmentCompleteness: 'complete',
  executionDomain: 'simulation',
  realEvidenceEligible: false,
  outcome: 'recovered',
  failureClass: 'execution_failure',
  evidenceId: 'sim-evidence',
  experienceIds: ['sim-exp'],
  previousFailureId: 'sim-fail',
...
assert.equal(
  await hasIndependentCrossSignal({
    skill: 'rdk-model-zoo',
    terminalEntries: [eligibleTerminal],
    crossSignals: [
      signal('stdout', 'execution-stdout', 'sha256:stdout'),
      signal('exit', 'process-exit', 'sha256:exit'),
    ],
  }),
  false,
  'two parsers in the execution group are not independent'
);
assert.equal(
  await hasIndependentCrossSignal({
    skill: 'rdk-model-zoo',
    ...
    crossSignals: [
      signal('stdout', 'execution-stdout', 'sha256:stdout'),
      signal('artifact', 'artifact-mime', 'sha256:artifact'),
    ],
  }),
  true
);
...
console.log(
  'self-evolution-boundaries: contracts, sim2real, multi-Skill attribution and cross-signal isolation ok'
);
