...
for (const variant of ['control', 'treatment']) {
  for (let i = 0; i < 20; i += 1) {
    ...
    await experimentLog.append({
      ...
      taskSignature: 'sig',
      variant,
      exposed: variant === 'treatment',
      exposureId: `exposure-${runId}`,
      hypothesis,
      experimentConfigHash: 'sha256:frozen',
      timestamp: new Date().toISOString(),
    });
    await experimentLog.append({
      schemaVersion: 1,
      ...
      success: true,
      retries: variant === 'control' ? 2 : 0,
      toolCalls: variant === 'control' ? 12 : 6,
      corrections: variant === 'control' ? 2 : 0,
      durationMs: variant === 'control' ? 2000 : 1000,
      inputTokens: variant === 'control' ? 2000 : 1000,
      outputTokens: 100,
      failureClasses: [],
      safetyFailed: false,
      eligible: true,
...
await experimentLog.append({
  ...
  variant: 'control',
  exposed: false,
  exposureId: 'exposure-mutated',
  hypothesis,
  experimentConfigHash: 'sha256:changed-after-outcomes',
  timestamp: new Date().toISOString(),
});
...
console.log(
  'cost-superiority experiment: noninferior success plus frozen multi-metric benefit activates'
);
