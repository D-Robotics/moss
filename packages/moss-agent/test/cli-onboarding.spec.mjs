 ...
 *   node packages/moss-agent/test/cli-onboarding.spec.mjs
 */
...
const workspacePath = path.resolve('/tmp/moss-workspace');
const runtimeDirPath = path.resolve('/tmp/moss-runtime');
const configDirPath = path.resolve('/tmp/moss-config');
...

{
  ...
  assert.doesNotMatch(quickStart, /restart moss/i);
}
