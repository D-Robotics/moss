 ...
 *   node packages/moss-agent/test/cli-model-env-isolation.spec.mjs
 */
...
const NO_BUNDLED = { MOSS_NO_BUNDLED_DEFAULT: '1' };
...
// 3) MOSS_* model vars are ignored too: config file wins, sources say config.
{
  const r = resolveCliConfig({
    ...NO_BUNDLED,
    MOSS_PROVIDER: 'openai',
    MOSS_MODEL: 'env-model',
    MOSS_BASE_URL: 'https://env.example.com',
    MOSS_API_KEY: 'env-key',
  }, {
    provider: 'qwen',
    model: 'config-model',
  ...
  console.log('  [PASS] MOSS_* model vars never override the config file');
}

// 4) MOSS_* model vars alone must not configure anything (no silent env-only setup).
{
  const r = resolveCliConfig({
    ...NO_BUNDLED,
    MOSS_PROVIDER: 'openai',
...
{
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-env-isolation-'));
  ...
  const r = resolveCliConfig({
    MOSS_BUNDLED_DEFAULT_FILE: gatewayFile,
...
// 6) Set-but-ignored model env vars are reported for doctor/startup messaging.
{
  ...
  assert.deepEqual(
    ...
    ['DEEPSEEK_API_KEY', 'MOSS_MODEL', 'OPENAI_BASE_URL'],
  );
