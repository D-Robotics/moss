#!/usr/bin/env node
/**
 * MossAgent goal mode integration tests.
 *
 * Run:
 *   npm run build -w @rdk-moss/agent
 *   node packages/moss-agent/test/moss-agent-goal-mode.spec.mjs
 */
...
assert.match(requests[0].systemPrompt, /<moss_goal_mode>/);
...
assert.doesNotMatch(requests[2].systemPrompt, /<moss_goal_mode>/);

const stored = await store.loadMessages(sessionKey);
assert.equal(
  JSON.stringify(stored).includes('moss_goal_checkpoint'),
  false,
  'goal checkpoints must not leak into active chat history after clearGoal()',
);

console.log('moss-agent goal mode tests passed');
