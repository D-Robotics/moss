 ...
 *   node packages/moss-agent/test/cli-install-skill-toolcall.spec.mjs
 */
...
test('CLI can execute an install_skill tool call and register the resulting SKILL.md', async () => {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-install-skill-cwd-'));
  const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-install-skill-config-'));
  let requestCount = 0;
  const requestBodies = [];
    ...
    const result = await new Promise((resolve, reject) => {
      const child = spawn(process.execPath, [
        ...
        env: {
          ...
          MOSS_TRUSTED_TOOLS: 'install_skill',
