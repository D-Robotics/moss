...
const modelDef = {
  id: 'plan-gate-ab', name: 'plan-gate-ab', api: 'openai-completions', provider: 'test',
...
function makeSlackoffProvider() {
  let turn = 0;
  return {
    id: 'plan-gate-ab', capabilities: { streaming: true },
    async complete(options) {
      ...
      if (turn === 1) {
        return { stopReason: 'tool_use', content: [
          ...
          { type: 'tool_use', id: 'c1', name: 'plan', input: { action: 'create', goal: 'g', steps: [{ description: 's1' }, { description: 's2' }, { description: 's3' }, { description: 's4' }, { description: 's5' }] } },
      ...
      if (turn === 3) {
        ...
        return { stopReason: 'tool_use', content: [
          { type: 'tool_use', id: 'd1', name: 'plan_step', input: { planId, stepNumber: 1, action: 'complete', actualOutput: 'done s1' } },
          { type: 'text', text: 'step1 done' },
        ], usage: { inputTokens: 1, outputTokens: 1 } };
      }
      // turn >= 4: SLACK OFF — plain end_turn claiming done with 4/5 unfinished.
      return { stopReason: 'end_turn', content: [{ type: 'text', text: 'All done!' }], usage: { inputTokens: 1, outputTokens: 1 } };
    },
...
async function run(gateFlag) {
  ...
  const agent = new MossAgent({
    llmProvider: makeSlackoffProvider(), sessionStore,
  ...
  let threw = null;
  try { await agent.chat(sessionKey, 'do it'); } catch (e) { threw = e; }
...
assert.equal(off.correctionInjected, false,
  'OFF: gate disabled -> no plan-completion correction injected (premature end_turn passed)');
assert.ok(off.threw === null || /Completion rejected/.test(off.threw?.message ?? ''),
...
assert.equal(on.correctionInjected, true,
  'ON (default): gate active -> plan-completion correction injected (premature end_turn rejected)');

console.log('[PASS] plan-completion-gate ab: off passes / on rejects (mechanism direction verified, not real gain)');
console.log('  OFF: correction injected =', off.correctionInjected, '| threw =', off.threw ? off.threw.message : 'none');
