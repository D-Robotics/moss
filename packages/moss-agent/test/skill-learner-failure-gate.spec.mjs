 ...
 *
 * Run:
 *   npm run build -w @rdk-moss/agent/skill-learning
 *   node packages/moss-skills/test/skill-learner-failure-gate.spec.mjs
 */
import assert from 'node:assert/strict';
import fs from 'node:fs/promises';
import os from 'node:os';
import path from 'node:path';
import { SkillLearner } from '../dist/skill-learning/skill-learner.js';
import { isCompactionSummaryText } from '../dist/skill-learning/llm-message.js';

const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-learner-gate-'));
...
// ── 2. compaction summary is not the user request ─────────────────
{
  ...
  const messages = [
    ...
    { role: 'user', content: '<moss_working_context_checkpoint version="1">state</moss_working_context_checkpoint>' },
