#!/usr/bin/env node
/**
 * Self-test for MossAgent tool-loop guard.
 *
 * Run:
 *   npm run build -w @rdk-moss/agent
 *   node packages/moss-agent/test/tool-loop-guard.spec.mjs
 *
...
const GUARD_MARKER = '[moss-agent] Tool loop guard stopped';
...
function runHighVolumeLocalToolGuardTests() {
  withEnv({
    MOSS_TOOL_LOOP_IDENTICAL_LIMIT: undefined,
    MOSS_TOOL_LOOP_SINGLE_TOOL_LIMIT: undefined,
    MOSS_TOOL_LOOP_TOTAL_LIMIT: undefined,
    MOSS_TOOL_LOOP_FAILURE_LIMIT: undefined,
  }, () => {
  ...
  withEnv({
    MOSS_TOOL_LOOP_IDENTICAL_LIMIT: 99,
    MOSS_TOOL_LOOP_SINGLE_TOOL_LIMIT: 3,
    MOSS_TOOL_LOOP_TOTAL_LIMIT: undefined,
  }, () => {
    const state = createToolLoopGuardState();
    for (let i = 0; i < 80; i += 1) {
  ...
  withEnv({
    MOSS_TOOL_LOOP_IDENTICAL_LIMIT: '0',
    MOSS_TOOL_LOOP_SINGLE_TOOL_LIMIT: undefined,
    MOSS_TOOL_LOOP_TOTAL_LIMIT: undefined,
    MOSS_TOOL_LOOP_FAILURE_LIMIT: 'off',
  }, () => {
    const state = createToolLoopGuardState();
    for (let i = 0; i < 8; i += 1) {
      assert.equal(
...
async function runStreamScenario() {
  const oldIdentical = process.env.MOSS_TOOL_LOOP_IDENTICAL_LIMIT;
  process.env.MOSS_TOOL_LOOP_IDENTICAL_LIMIT = '2';
  try {
    ...
    if (oldIdentical === undefined) delete process.env.MOSS_TOOL_LOOP_IDENTICAL_LIMIT;
    else process.env.MOSS_TOOL_LOOP_IDENTICAL_LIMIT = oldIdentical;
  }
}
...
await runChatScenario(
  ...
  { MOSS_TOOL_LOOP_IDENTICAL_LIMIT: 2 },
...
await runChatScenario(
  ...
  {
    MOSS_TOOL_LOOP_IDENTICAL_LIMIT: 99,
    MOSS_TOOL_LOOP_SINGLE_TOOL_LIMIT: 99,
    MOSS_TOOL_LOOP_TOTAL_LIMIT: 4,
  },
  ({ calls, messages }) => {
...
await runChatScenario(
  ...
  {
    MOSS_TOOL_LOOP_IDENTICAL_LIMIT: 'not-a-number',
    MOSS_TOOL_LOOP_SINGLE_TOOL_LIMIT: undefined,
    MOSS_TOOL_LOOP_TOTAL_LIMIT: undefined,
    MOSS_TOOL_LOOP_FAILURE_LIMIT: undefined,
  },
...
async function runFailureLoopScenario() {
  const env = {
    ...
    MOSS_TOOL_LOOP_SINGLE_TOOL_LIMIT: '99',
    MOSS_TOOL_LOOP_TOTAL_LIMIT: '99',
    MOSS_TOOL_LOOP_FAILURE_LIMIT: '3',
  };
  const prev = {};
