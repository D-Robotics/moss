 ...
 * Run: npm run build -w @rdk-moss/agent/skill-learning && node packages/moss-skills/test/skill-pipeline-quality-gate.spec.mjs
 */
...
{
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-gate-a-'));
...
{
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-gate-b-'));
  const pipeline = new SkillPipeline({ workspaceDir: tmp, model: 'test' });
  const messages = makeMessages(
    [
...
{
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-gate-c-'));
  const pipeline = new SkillPipeline({ workspaceDir: tmp, model: 'test' });
  const messages = makeMessages(
    [
      { name: 'read_file', input: { path: '/a' } },
...
{
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-gate-d-'));
  const pipeline = new SkillPipeline({ workspaceDir: tmp, model: 'test' });
  const messages = makeMessages(
    [
