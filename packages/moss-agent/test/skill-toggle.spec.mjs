...
try {
  ...
  assert.ok(
    before.some((s) => s.name === 'code-review'),
  ...
  assert.ok(!after.some((s) => s.name === 'code-review'), 'disabled code-review no longer matches');

  // And it must NOT be surfaced as a /command.
  const cmds = loadSkillCommands(registry, new Set());
  assert.ok(!cmds.some((c) => c.name === '/code-review'), 'disabled code-review is not a /command');
