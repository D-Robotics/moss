 ...
 *   node packages/moss-agent/test/cli-provider-routing.spec.mjs
 */
...
function writeProbeConfig(modelConfig) {
  const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-provider-routing-'));
  fs.writeFileSync(
...
function runProviderProbe(modelConfig) {
  const script = `
    ...
    const { cliProvider } = await import('./packages/moss-agent/dist/cli/providers.js');
  ...
  return spawnSync(process.execPath, ['--input-type=module', '-e', script], {
    ...
    env: {
      ...
      HOME: os.homedir(),
      MOSS_CONFIG_DIR: writeProbeConfig(modelConfig),
...

{
  ...
  const result = spawnSync(process.execPath, ['--input-type=module', '-e', script], {
    ...
    env: {
      PATH: process.env.PATH,
      HOME: os.homedir(),
      MOSS_CONFIG_DIR: writeProbeConfig({
        provider: 'openai-compatible',
