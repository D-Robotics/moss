 ...
 *   node packages/moss-agent/test/cli-config-setup.spec.mjs
 */
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-config-'));
const oldConfigDir = process.env.MOSS_CONFIG_DIR;
const oldConfigFile = process.env.MOSS_CONFIG_FILE;
const oldConfigPath = process.env.MOSS_CONFIG_PATH;
process.env.MOSS_CONFIG_DIR = tmp;
...
try {
  ...
  const envResolved = resolveCliConfig({
    MOSS_PROFILE: 'autonomous',
    MOSS_PROVIDER: 'openai',
    OPENAI_API_KEY: 'env-secret',
    MOSS_MODEL: 'gpt-4o-mini',
    MOSS_BASE_URL: 'https://api.openai.com',
    MOSS_SAFETY_MODE: 'read-only',
    MOSS_APPROVAL_POLICY: 'never',
    MOSS_TRUSTED_TOOLS: 'exec,filesystem__*',
    MOSS_DENIED_TOOLS: 'device_*',
    MOSS_PROMPT_CACHE: 'false',
    MOSS_PROMPT_CACHE_DEBUG: 'true',
    MOSS_MCP_ENABLED: 'true',
    MOSS_MCP_CONFIG: '/tmp/moss-mcp-env.json',
    MOSS_MAX_AGENT_TURNS: '12',
    MOSS_CONTEXT_TOKENS: '64000',
    MOSS_IMAGE_INPUT: 'true',
  }, loadConfigFile());
  ...
  assert.equal(envResolved.profileSource, 'MOSS_PROFILE');
  ...
  assert.deepEqual(
    ...
    ['MOSS_BASE_URL', 'MOSS_MODEL', 'MOSS_PROVIDER', 'OPENAI_API_KEY'],
  );
  assert.equal(envResolved.safetyMode, 'read-only');
  assert.equal(envResolved.safetyModeSource, 'MOSS_SAFETY_MODE');
  assert.equal(envResolved.approvalPolicy, 'never');
  assert.equal(envResolved.approvalPolicySource, 'MOSS_APPROVAL_POLICY');
  assert.deepEqual(envResolved.trustedTools, ['exec', 'filesystem__*']);
  assert.equal(envResolved.trustedToolsSource, 'MOSS_TRUSTED_TOOLS');
  assert.deepEqual(envResolved.deniedTools, ['device_*']);
  assert.equal(envResolved.deniedToolsSource, 'MOSS_DENIED_TOOLS');
  assert.equal(envResolved.promptCacheEnabled, false);
  assert.equal(envResolved.promptCacheSource, 'MOSS_PROMPT_CACHE');
  assert.equal(envResolved.promptCacheDebug, true);
  assert.equal(envResolved.promptCacheDebugSource, 'MOSS_PROMPT_CACHE_DEBUG');
  assert.equal(envResolved.mcpEnabled, true);
  assert.equal(envResolved.mcpEnabledSource, 'MOSS_MCP_ENABLED');
  assert.equal(envResolved.mcpConfigPath, '/tmp/moss-mcp-env.json');
  assert.equal(envResolved.mcpConfigPathSource, 'MOSS_MCP_CONFIG');
  assert.equal(envResolved.maxAgentTurns, 12);
  assert.equal(envResolved.maxAgentTurnsSource, 'MOSS_MAX_AGENT_TURNS');
  assert.equal(envResolved.contextTokens, 64000);
  assert.equal(envResolved.contextTokensSource, 'MOSS_CONTEXT_TOKENS');
  assert.equal(envResolved.imageInput, true);
  assert.equal(envResolved.imageInputSource, 'MOSS_IMAGE_INPUT');
  ...
  const invalidEnvResolved = resolveCliConfig({
    MOSS_MAX_AGENT_TURNS: '1.5',
    MOSS_CONTEXT_TOKENS: '64000x',
  }, loadConfigFile());
  ...
  assert.equal(redactedJson.schema, 'moss_cli_config.v1');
  ...
  assert.match(usage, /MOSS_CONFIG_FILE/);
  ...
  for (const args of [['config', '--json'], ['config', 'show', '--json']]) {
    ...
    assert.equal(parsed.schema, 'moss_cli_config.v1');
  ...
  assert.equal(validateJson.schema, 'moss_cli_config_validation.v1');
  ...
  const initConfigDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-init-config-'));
  const oldInitConfigDir = process.env.MOSS_CONFIG_DIR;
  const oldInitConfigFile = process.env.MOSS_CONFIG_FILE;
  const initEnvNames = [
    'MOSS_PROFILE',
    'MOSS_PROVIDER',
    'MOSS_API_KEY',
  ...
  const oldNoBundled = process.env.MOSS_NO_BUNDLED_DEFAULT;
  try {
    process.env.MOSS_CONFIG_DIR = initConfigDir;
    delete process.env.MOSS_CONFIG_FILE;
    ...
    if (oldInitConfigDir === undefined) delete process.env.MOSS_CONFIG_DIR;
    else process.env.MOSS_CONFIG_DIR = oldInitConfigDir;
    if (oldInitConfigFile === undefined) delete process.env.MOSS_CONFIG_FILE;
    else process.env.MOSS_CONFIG_FILE = oldInitConfigFile;
    if (oldNoBundled === undefined) delete process.env.MOSS_NO_BUNDLED_DEFAULT;
    else process.env.MOSS_NO_BUNDLED_DEFAULT = oldNoBundled;
  ...
  const initProjectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-init-project-'));
  ...
  const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-project-config-'));
  const projectChild = path.join(projectRoot, 'src', 'feature');
  const projectConfigDir = path.join(projectRoot, '.moss');
  ...
  try {
    ...
    const loadedProject = loadCliConfigFile({ MOSS_CONFIG_DIR: tmp }, [], projectChild);
    ...
    const projectResolved = resolveCliConfig({ MOSS_CONFIG_DIR: tmp }, loadedProject.config, {}, loadedProject);
    ...
    const projectOnlyConfigDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-project-only-config-'));
    try {
      const projectOnlyLoaded = loadCliConfigFile({ MOSS_CONFIG_DIR: projectOnlyConfigDir }, [], projectChild);
      const projectOnlyResolved = resolveCliConfig({ MOSS_CONFIG_DIR: projectOnlyConfigDir }, projectOnlyLoaded.config, {}, projectOnlyLoaded);
    ...
    const explicitLoaded = loadCliConfigFile({ MOSS_CONFIG_DIR: tmp, MOSS_CONFIG_FILE: explicitConfigPath }, [], projectChild);
  ...
  const newProjectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-new-project-config-'));
  try {
  ...
  const explicitDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-explicit-config-'));
  ...
  process.env.MOSS_CONFIG_FILE = explicitConfigPath;
  try {
    ...
    assert.equal(resolveConfigPath(undefined, { MOSS_CONFIG_FILE: envIgnoredConfigPath }, ['--config-file', cliConfigPath]), cliConfigPath);
    saveConfigFile({
      profile: 'cautious',
      provider: 'openai',
      apiKey: 'file-secret',
    ...
    const explicitResolved = resolveCliConfig({ MOSS_CONFIG_FILE: explicitConfigPath }, loadConfigFile());
    ...
    const result = spawnSync(process.execPath, [cliPath, 'config', 'show'], {
      env: {
        ...process.env,
        MOSS_CONFIG_FILE: explicitConfigPath,
        NO_COLOR: '1',
      },
      encoding: 'utf8',
    });
    ...
    const setResult = spawnSync(process.execPath, [
      ...
      env: {
        ...process.env,
        MOSS_CONFIG_FILE: envIgnoredConfigPath,
        NO_COLOR: '1',
      },
      encoding: 'utf8',
    });
    ...
    assert.throws(
      ...
      (err) => err instanceof CliConfigFileError &&
        ...
        /Invalid moss config/.test(err.message),
    );
    ...
    assert.match(invalidShowResult.stderr, /Invalid moss config/);
    ...
    assert.match(invalidValidateResult.stderr, /Invalid moss config/);
    ...
    if (oldConfigFile === undefined) delete process.env.MOSS_CONFIG_FILE;
    else process.env.MOSS_CONFIG_FILE = oldConfigFile;
  ...
  saveConfigFile({
    ...
    mcp: { enabled: true, configPath: '/tmp/moss-mcp.json' },
    guardrails: {
      input: {
  ...
  assert.equal(filePolicyResolved.mcpConfigPath, '/tmp/moss-mcp.json');
  ...
  runConfigSet(['workspace', '/tmp/moss-config-workspace']);
  assert.equal(loadConfigFile().workspace, path.resolve('/tmp/moss-config-workspace'));
  ...
  assert.deepEqual(loadConfigFile().mcp, { enabled: true, configPath: '/tmp/moss-mcp.json' });
  ...
  if (oldConfigDir === undefined) delete process.env.MOSS_CONFIG_DIR;
  else process.env.MOSS_CONFIG_DIR = oldConfigDir;
  if (oldConfigFile === undefined) delete process.env.MOSS_CONFIG_FILE;
  else process.env.MOSS_CONFIG_FILE = oldConfigFile;
  if (oldConfigPath === undefined) delete process.env.MOSS_CONFIG_PATH;
  else process.env.MOSS_CONFIG_PATH = oldConfigPath;
