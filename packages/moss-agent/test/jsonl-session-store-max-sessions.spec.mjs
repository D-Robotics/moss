 ...
 *   node packages/moss-agent/test/jsonl-session-store-max-sessions.spec.mjs
 */
...
{
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-maxsess-default-'));
...
{
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-maxsess-cap-'));
...
{
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-maxsess-reuse-'));
