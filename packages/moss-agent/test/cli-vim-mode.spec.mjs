#!/usr/bin/env node
/**
 * Vim modal handler: MOSS_VIM_MODE actually enables it, NORMAL/INSERT
...
delete process.env.MOSS_VIM_MODE;
assert.equal(isVimEnabled(), false);
assert.deepEqual(handleVimKey('i', 0, 0), { type: 'none' });

// Enabled via env (the "MOSS_VIM_MODE=1 actually enables it" contract).
process.env.MOSS_VIM_MODE = '1';
