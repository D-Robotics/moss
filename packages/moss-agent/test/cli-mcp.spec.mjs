 ...
 *   node packages/moss-agent/test/cli-mcp.spec.mjs
 */
...

{
  ...
  const connections = await registerConfiguredMcpTools(agent, {
    mcpEnabled: false,
    mcpConfigPath: '/tmp/moss-mcp-disabled.json',
  });
...
{
  const dir = mkdtempSync(join(tmpdir(), 'moss-cli-mcp-'));
...

{
  ...
  try {
    const connections = await registerConfiguredMcpTools(agent, {
      mcpEnabled: true,
      mcpConfigPath: '/tmp/moss-missing-mcp-config.json',
    });
    assert.deepEqual(connections, []);
    assert.equal(agent.tools.size, 0);
