 ...
 *   node packages/moss-agent/test/zero-config-pack.spec.mjs
 */
...
try {
  ...
  const generated = spawnSync(process.execPath, [prepareScript, '--package-dir', tmp], {
    env: {
      ...process.env,
      MOSS_ZERO_CONFIG_DEFAULT_JSON: JSON.stringify(packConfig),
    },
    encoding: 'utf8',
  });
  ...
  const missing = spawnSync(process.execPath, [prepareScript, '--package-dir', tmp], {
    env: {
      ...
      MOSS_ZERO_CONFIG_PROVIDER: '',
      MOSS_ZERO_CONFIG_MODEL: '',
      MOSS_ZERO_CONFIG_BASE_URL: '',
      MOSS_ZERO_CONFIG_API_KEY: '',
    },
    encoding: 'utf8',
  });
