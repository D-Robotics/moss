 ...
 *   node packages/moss-agent/test/cli-resume-replay.spec.mjs
 */
...
assert.equal(
  resumedMessageText({ role: 'user', content: '<moss_working_context_checkpoint>{...}</moss_working_context_checkpoint>' }),
  '',
);
...
// buildResumeReplay: keeps user/assistant text turns in order, drops tool/checkpoint/empty.
{
  const messages = [
    ...
    { role: 'user', content: '<moss_working_context_checkpoint>x</moss_working_context_checkpoint>' }, // dropped
