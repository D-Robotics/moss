...
const originalPort = process.env.MOSS_DEVICE_PORT;
const originalHost = process.env.MOSS_DEVICE_HOST;

try {
  // Valid port
  process.env.MOSS_DEVICE_HOST = '10.0.0.1';
  process.env.MOSS_DEVICE_PORT = '22';
  const config1 = getDeviceConfigFromEnv();
  assert.equal(config1.port, 22);
  console.log('  [PASS] valid port 22 accepted');

  // Port out of range
  process.env.MOSS_DEVICE_PORT = '999999';
  ...
  // Negative port
  process.env.MOSS_DEVICE_PORT = '-1';
  ...
  process.env.MOSS_DEVICE_PORT = originalPort;
  process.env.MOSS_DEVICE_HOST = originalHost;
}
