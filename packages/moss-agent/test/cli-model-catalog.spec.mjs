 ...
 *   node packages/moss-agent/test/cli-model-catalog.spec.mjs
 */
...
const builtIn = await loadModelChoicesForRuntime({
  ...
  configPath: '/tmp/moss/config.json',
}, 'Moss');
...
assert.match(formatModelChoices(builtIn), /config file\s+\/tmp\/moss\/config\.json/);
