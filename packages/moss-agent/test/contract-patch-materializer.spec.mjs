...
await fs.writeFile(path.join(skillDir, 'SKILL.md'), [
  '---', 'name: test-skill', 'description: test', 'version: 1.0.0', '---', '', '# Test',
].join('\n'));
const acceptancePath = path.join(skillDir, 'ACCEPTANCE.json');
await fs.writeFile(acceptancePath, `${JSON.stringify({
  skillName: 'test-skill', version: '1', expectedTools: ['exec'],
...
const materializer = new ContractPatchMaterializer({
  workspaceDir: workspace, patchLog,
  skillRegistry: new SkillRegistry({ workspaceDir: workspace, includeBuiltin: false, includeBundledRdkSkills: false }),
});
const proposal = {
  skill: 'test-skill', section: 'postconditions',
  spec: { name: 'stdout_matches', params: { pattern: 'READY' } },
  expectedVersion: '1', sourceEventIds: ['learning-1'], environmentFingerprint: 'sha256:test',
};
const candidate = { id: 'term_test-skill', targetSkill: 'test-skill', provenance: { layer: 'L2', kind: 'explicit-proposal', source: 'test', proposalRef: 'test://proposal' } };
const rejected = await materializer.publish(proposal, {
  candidate, decision: { promotable: false, statisticalPassed: true, crossSignalPassed: false, reason: 'no cross signal' },
});
...
const published = await materializer.publish(proposal, {
  candidate, decision: { promotable: true, statisticalPassed: true, crossSignalPassed: true, reason: 'both gates passed' },
});
...
assert.equal(await materializer.rollback(published.id), true, 'a later rejected attempt does not hide the last published revision');
assert.equal(JSON.parse(await fs.readFile(acceptancePath, 'utf8')).version, '1');

const rollbackProposal = { ...proposal, spec: { name: 'stdout_matches', params: { pattern: 'HEALTHY' } } };
...
assert.equal(JSON.parse(await fs.readFile(acceptancePath, 'utf8')).version, '1', 'rollback restores the previous contract');

await fs.rm(workspace, { recursive: true, force: true });
console.log('contract-patch-materializer: whitelist, ownership, promotion gates, version check and backup ok');
