...
assert.equal(resolveCliSafetyMode([], { MOSS_SAFETY_MODE: 'full' }), 'full-access');
...
{
  const approve = createCliToolApprovalHook('workspace-write', { MOSS_CLI_AUTO_APPROVE: '1' });
  assert.deepEqual(
    await approve({
      tool: tool('exec', 'local_write', 'requires_user_confirmation'),
      input: { command: 'npm test' },
...
{
  const approve = createCliToolApprovalHook('full-access', { MOSS_CLI_AUTO_APPROVE: '1' });
  assert.deepEqual(
    await approve({
      tool: tool('device_exec', 'device_mutation', 'requires_user_confirmation'),
      input: { command: 'uptime' },
...
{
  const approve = createCliToolApprovalHook('read-only', { MOSS_CLI_AUTO_APPROVE: '1' });
  const denied = await approve({
    tool: tool('exec', 'local_write', 'requires_user_confirmation'),
    input: { command: 'npm test' },
    sessionKey: 's',
