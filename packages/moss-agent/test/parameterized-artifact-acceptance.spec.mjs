...
await fs.writeFile(
  artifact,
  Buffer.from([0xff, 0xd8, ...Array.from({ length: 4096 }, (_, i) => i % 251), 0xff, 0xd9])
);
const base = {
  result: '',
...
assert.equal(
  (
    await evaluatePredicate(
      { name: 'artifact_digest_changed', params: { path: artifact, previousDigest: digest } },
      base
    )
  ).reasonCode,
...
assert.equal(
  (
    await evaluatePredicate(
      { name: 'image_content_nontrivial', params: { path: artifact, minVariation: 20 } },
      base
    )
  ).verdict,
  'pass'
...
assert.deepEqual(
  validateAcceptSpecs([
    { name: 'image_dimensions', params: { path: artifact, width: 1, height: 1 } },
  ]),
  []
);
assert.ok(
  validateAcceptSpecs([
    { name: 'image_dimensions', params: { path: artifact, width: 0, height: 1 } },
  ]).length > 0
);
