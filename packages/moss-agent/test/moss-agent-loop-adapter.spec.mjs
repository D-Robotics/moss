#!/usr/bin/env node
/**
 * Self-test for legacy MossAgent adapter helpers.
 *
 * Run:
 *   npm run build -w @rdk-moss/agent
 *   node packages/moss-agent/test/moss-agent-loop-adapter.spec.mjs
 */
...
{
  const model = createModelDefFromMossConfig({
  ...
  const thinkingHistoryModel = createModelDefFromMossConfig({
    llmProvider: {
      id: 'fake-provider',
      displayName: 'Fake',
      complete: async () => null,
...
console.log('[PASS] MossAgent loop adapter maps MiniAgent events and versions streams');
