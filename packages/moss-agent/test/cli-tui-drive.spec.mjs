#!/usr/bin/env node
/**
 * Drive the REAL MossTui like a human (ink-testing stdin) and assert the behaviors
 ...
 *
 * Run: npm run build -w @rdk-moss/agent && node packages/moss-agent/test/cli-tui-drive.spec.mjs
 */
...
import { MossTui } from '../dist/cli/tui.js';
...
const runtime = {
  ...
  configDir: '/tmp/moss-test/config',
...
const mount = (agent = makeAgent(), runtimeOverride = runtime) => render(React.createElement(
  ...
  React.createElement(MossTui, { agent, skillLearner, runtime: runtimeOverride, sessionKey: 'cli' }),
));
...
function assertNoCrush(frame, ctx) {
  ...
  assert.doesNotMatch(s, /moss-agent\s*│?\s*(model:|>_)/, `${ctx}: cwd line merged into another (squash)`);
}
...
test('/mcp lists configured servers with tool counts and a connected/failed marker', async () => {
  ...
  const mcpRuntime = {
    ...runtime,
    config: { mcpEnabled: true, mcpConfigPath: '/tmp/moss-test/mcp.json' },
    mcp: [
