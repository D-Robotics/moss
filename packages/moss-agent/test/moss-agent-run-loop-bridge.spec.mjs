#!/usr/bin/env node
/**
 * Self-test for the MossAgent.streamChat → runAgentLoop bridge.
 *
 * Run:
 *   npm run build -w @rdk-moss/agent
 *   node packages/moss-agent/test/moss-agent-run-loop-bridge.spec.mjs
 */
...
import { MossAgent, InMemorySessionStore, JsonlSessionStore } from '../dist/core/index.js';
...

{
  ...
  assert(!requests[0].systemPromptParts.stable.includes('<moss_working_context>'));
  assert(requests[0].systemPromptParts.dynamic.includes('<moss_working_context>'));
  assert.equal(
...

{
  ...
  assert(requests[0].systemPrompt.includes('base'));
  assert(requests[0].systemPrompt.includes('<moss_working_context>'));
}

{
  const usageDir = await mkdtemp(path.join(os.tmpdir(), 'moss-observability-bridge-'));
  const origUsageLog = process.env.MOSS_LLM_USAGE_LOG;
  process.env.MOSS_LLM_USAGE_LOG = path.join(usageDir, 'llm-usage.jsonl');
  ...
  try {
    ...
    assert(llmTurnSpan, 'expected real MossAgent.chat() to start agent.llm_turn span');
    ...
    if (origUsageLog) {
      process.env.MOSS_LLM_USAGE_LOG = origUsageLog;
    } else {
      delete process.env.MOSS_LLM_USAGE_LOG;
    }
...
{
  const usageDir = await mkdtemp(path.join(os.tmpdir(), 'moss-llm-usage-bridge-'));
  const origUsageLog = process.env.MOSS_LLM_USAGE_LOG;
  process.env.MOSS_LLM_USAGE_LOG = path.join(usageDir, 'llm-usage.jsonl');
...
{
  const dir = await mkdtemp(path.join(os.tmpdir(), 'moss-bridge-jsonl-'));
  try {
...
console.log('[PASS] MossAgent runAgentLoop bridge streams text and tools');
