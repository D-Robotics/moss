...
if (process.env.MOSS_REALBOARD_TEST !== '1' || !host) {
  console.log(
    '  [SKIP] realboard-priority-contracts: set MOSS_REALBOARD_TEST=1 and MOSS_REALBOARD_HOST to opt in'
  );
  process.exit(0);
}
...
const contracts = ContractRegistry.fromSkills(
  new SkillRegistry({ workspaceDir: workspace }).list()
);
...
async function assertContract(skill, command) {
  ...
  const result = await evaluatePostconditions(contract.postconditions, {
    ...
    reportedIsError: executed.exitCode !== 0,
    input: { command },
    workspaceDir: workspace,
    deviceExecutor: readonly,
  });
...
await assertContract(
  'rdk-isp-tuning',
  '/app/multimedia_samples/sample_isp/get_isp_data/get_isp_data -h'
);
...
const created = await planTool.execute(
  {
    action: 'create',
    goal: 'safe RDK X5 capture verification',
    steps: [
      {
        description: 'convert fresh stable frame',
        expectedTools: ['device_exec'],
        expectedAccept: ['rdk-capture-photo'],
      },
    ],
    terminalAccept: [
...
await experienceLog.append({
  ...
  attemptId: `${runId}:capture-evidence`,
  toolCallId: 'capture-evidence',
  ...
  tool: 'device_exec',
  input: { command: 'ffmpeg capture to /tmp/photo.jpg' },
  reportedIsError: false,
  verdict: 'pass',
  reasonCode: 'capture_verified',
...
const gate = wrapWithTerminalArbitration(async () => ({ ok: true }), {
  ...
  crossSignalLog: crossLog,
  planProvider: { get: getActivePlanForSession },
  workspaceDir: workspace,
  deviceExecutor: { current: readonly },
});
assert.equal(
  (
    await gate({
...
assert.equal(
  await hasIndependentCrossSignal({
    ...
    terminalEntries: await terminalLog.readAll(),
    crossSignals: await crossLog.readAll(),
  }),
  true
);
...
await session.close();
console.log(
  `realboard-priority-contracts: X5 contracts and independent capture signal passed (${identity.fingerprint})`
);
