#!/usr/bin/env node
/**
 * @rdk-moss/agent/memory — memory-context-selector unit tests
 *
 * Run after package build:
 *   npm run build -w @rdk-moss/agent/memory && node packages/moss-memory/test/memory-context-selector.spec.mjs
 */
...
const base = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-mem-ctx-'));
