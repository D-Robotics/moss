...
// 3. MOSS_ROS_DOMAIN_ID flows from env into the device config.
const savedHost = process.env.MOSS_DEVICE_HOST;
const savedDomain = process.env.MOSS_ROS_DOMAIN_ID;
try {
  process.env.MOSS_DEVICE_HOST = '10.0.0.9';
  process.env.MOSS_ROS_DOMAIN_ID = '7';
  assert.equal(getDeviceConfigFromEnv().rosDomainId, 7);

  process.env.MOSS_ROS_DOMAIN_ID = '';
  assert.equal(getDeviceConfigFromEnv().rosDomainId, undefined, 'blank domain is ignored');

  process.env.MOSS_ROS_DOMAIN_ID = 'not-a-number';
  assert.equal(getDeviceConfigFromEnv().rosDomainId, undefined, 'invalid domain is ignored');
} finally {
  if (savedHost === undefined) delete process.env.MOSS_DEVICE_HOST; else process.env.MOSS_DEVICE_HOST = savedHost;
  if (savedDomain === undefined) delete process.env.MOSS_ROS_DOMAIN_ID; else process.env.MOSS_ROS_DOMAIN_ID = savedDomain;
}
