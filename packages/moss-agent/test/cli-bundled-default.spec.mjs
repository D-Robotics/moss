 ...
 *   node packages/moss-agent/test/cli-bundled-default.spec.mjs
 */
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-bundled-'));
...
{
  const r = resolveCliConfig({ MOSS_BUNDLED_DEFAULT_FILE: gatewayFile }, {});
...
{
  const r = resolveCliConfig({ MOSS_BUNDLED_DEFAULT_FILE: gatewayFile }, { apiKey: 'user-key' });
...
{
  const r = resolveCliConfig({ MOSS_BUNDLED_DEFAULT_FILE: gatewayFile, MOSS_API_KEY: 'env-key' }, {});
  assert.equal(r.usingBundledDefault, true);
  assert.equal(r.apiKey, 'gw-token-test');
  assert.deepEqual(r.ignoredModelEnvVars, ['MOSS_API_KEY']);
  console.log('  [PASS] env keys neither shadow the bundled default nor become the key');
}

// 3) MOSS_NO_BUNDLED_DEFAULT=1 disables the bundled default.
{
  const r = resolveCliConfig({ MOSS_BUNDLED_DEFAULT_FILE: gatewayFile, MOSS_NO_BUNDLED_DEFAULT: '1' }, {});
  assert.notEqual(r.model, 'GatewayModel');
  assert.notEqual(r.apiKey, 'gw-token-test');
  console.log('  [PASS] MOSS_NO_BUNDLED_DEFAULT disables the bundled default');
}

// 4) A missing bundled file falls back gracefully (no throw, provider default).
{
  const r = resolveCliConfig({ MOSS_BUNDLED_DEFAULT_FILE: path.join(tmp, 'absent.json') }, {});
