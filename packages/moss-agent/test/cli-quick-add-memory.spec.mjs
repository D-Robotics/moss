#!/usr/bin/env node
/**
 * `#text` quick-add memory: drives the real MossTui and asserts the line is
...
const ws = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-quickmem-'));
try {
  const { stdin, lastFrame } = render(React.createElement(MossTui, {
