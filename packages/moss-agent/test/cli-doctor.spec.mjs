 ...
 *   node packages/moss-agent/test/cli-doctor.spec.mjs
 */
...
function resolvedConfig(overrides = {}) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-doctor-config-'));
  return {
...
{
  const fixture = resolvedConfig({
    ignoredModelEnvVars: ['DEEPSEEK_API_KEY', 'MOSS_PROVIDER'],
  });
  try {
    const output = await doctor(fixture.config);
    assert.match(output, /warn\s+env ignored: DEEPSEEK_API_KEY, MOSS_PROVIDER/);
...

{
  const fixture = resolvedConfig({
    mcpEnabled: true,
    mcpEnabledSource: 'MOSS_MCP_ENABLED',
    mcpConfigPathSource: 'MOSS_MCP_CONFIG',
  });
  try {
    const output = await doctor(fixture.config);
    assert.match(output, /fail\s+mcp: enabled \(MOSS_MCP_ENABLED\) but config is missing at .*mcp\.json/);
    assert.match(output, /warn\s+env overrides: MOSS_MCP_ENABLED, MOSS_MCP_CONFIG/);
  } finally {
    fixture.cleanup();
  }
}
