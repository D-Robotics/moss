...
import { getActivePlanForSession, getPlanController } from '../dist/plan-execute/plan-controller-store.js';
...
{
  const out = await planTool.execute({ action: 'create', goal: 'software task', steps: [{ description: 'edit code', expectedTools: ['edit_file'] }] }, ctx('ordinary'));
  const id = planId(out);
  assert.ok(id);
  assert.match(out, /not eligible for Promotion evidence/);
  assert.doesNotMatch(await planTool.execute({ action: 'review', planId: id }, ctx('ordinary')), /machine acceptance is incomplete/);
}

// Device execution creates a draft, but review/approve/start all reject missing evidence fields.
{
  const out = await planTool.execute({ action: 'create', goal: 'run on board', steps: [{ description: 'run', expectedTools: ['device_exec'] }] }, ctx('incomplete'));
  const id = planId(out);
  assert.ok(id);
...
// Invalid contract references and terminal predicate payloads are rejected at create.
{
  ...
  const badName = await planTool.execute({ action: 'create', goal: 'bad predicate', steps: [{ description: 'x' }], terminalAccept: [{ name: 'invented', params: {} }] }, ctx('bad-name'));
  assert.match(badName, /not a supported acceptance predicate/);
  const missing = await planTool.execute({ action: 'create', goal: 'missing parameter', steps: [{ description: 'x' }], terminalAccept: [{ name: 'file_exist', params: {} }] }, ctx('missing'));
  assert.match(missing, /params.path/);
  const numeric = await planTool.execute({ action: 'create', goal: 'bad number', steps: [{ description: 'x' }], terminalAccept: [{ name: 'video_fps_above', params: { threshold_fps: Number.NaN, readCommand: 'x', valueRegex: 'x' } }] }, ctx('numeric'));
  assert.match(numeric, /finite number/);
  const regex = await planTool.execute({ action: 'create', goal: 'bad regex', steps: [{ description: 'x' }], terminalAccept: [{ name: 'stdout_matches', params: { pattern: '[' } }] }, ctx('regex'));
...
// Real plan tool input persists and exposes step contracts and terminal predicates.
{
  ...
  const out = await planTool.execute({
    action: 'create', goal: 'trusted board run',
...
console.log('plan-machine-acceptance: real tool input, validation, enforcement, persistence, format/status ok');
