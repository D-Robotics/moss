 ...
 *   node packages/moss-agent/test/cli-community-auth.spec.mjs
 */
...
async function runLoginCallbackTest(buildCallbackUrl) {
  const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-community-auth-callback-'));
  const lines = [];
  const loginPromise = runMossCommunityAuthLogin({
    configDir,
    fetchImpl: mockFetch(),
    openBrowser: false,
  ...
  assert.equal(getMossCommunityAuthStatus({ configDir }).authenticated, true);
  assert.equal(readMossCommunityAuthSession(configDir)?.user.email, 'tester@example.com');
  assert.equal(clearMossCommunityAuthSession(configDir), true);
}
...
{
  const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-community-auth-manual-'));
  const lines = [];
  let prompted = false;
...
{
  const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-community-auth-state-mismatch-'));
  const lines = [];
  const loginPromise = runMossCommunityAuthLogin({
    configDir,
    fetchImpl: mockFetch(),
...

{
  ...
  assert.doesNotMatch(message, /start moss again/i);
}

{
  const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-community-auth-optional-'));
