 ...
 *   node packages/moss-agent/test/browser-tools.spec.mjs
 */
...
const oldExecutable = process.env.MOSS_BROWSER_EXECUTABLE;
process.env.MOSS_BROWSER_EXECUTABLE = '/definitely/missing/chromium-for-moss-test';
try {
  ...
  assert.match(missingBrowserResult, /MOSS_BROWSER_EXECUTABLE|Chromium|Chrome|browser/i);
} finally {
  if (oldExecutable === undefined) {
    delete process.env.MOSS_BROWSER_EXECUTABLE;
  } else {
    process.env.MOSS_BROWSER_EXECUTABLE = oldExecutable;
  }
}

const liveExecutable = process.env.MOSS_BROWSER_LIVE_EXECUTABLE || process.env.MOSS_BROWSER_EXECUTABLE;
