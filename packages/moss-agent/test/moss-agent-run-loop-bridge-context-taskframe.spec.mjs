#!/usr/bin/env node
/**
 * Parity coverage for the MossAgent.streamChat -> runAgentLoop bridge:
 * working-context checkpoints, TaskFrame resumability, max-turn pressure, and
 * overflow compaction hooks.
 *
 * Run:
 *   npm run build -w @rdk-moss/agent
 *   node packages/moss-agent/test/moss-agent-run-loop-bridge-context-taskframe.spec.mjs
 */
...
function checkpointMessages(messages) {
  return messages.filter((message) =>
    JSON.stringify(message).includes('moss_working_context_checkpoint'),
  );
}
...
function parseCheckpoint(message) {
  ...
  const match = text.match(
    /<moss_working_context_checkpoint[^>]*>\s*([\s\S]*?)\s*<\/moss_working_context_checkpoint>/,
  );
...
{
  const dir = await mkdtemp(path.join(os.tmpdir(), 'moss-bridge-overflow-jsonl-'));
...
console.log('[PASS] MossAgent bridge context and TaskFrame parity tests');
