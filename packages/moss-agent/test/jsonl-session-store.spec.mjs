 ...
 *   node packages/moss-agent/test/jsonl-session-store.spec.mjs
 */
...
const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-jsonl-session-'));
...
{
  const collisionDir = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-jsonl-session-collision-'));
...
{
  const legacyDir = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-jsonl-session-legacy-'));
...
{
  const malformedDir = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-jsonl-session-malformed-'));
...
{
  const traversalDir = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-jsonl-session-traversal-'));
