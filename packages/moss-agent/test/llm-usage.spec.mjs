 ...
 *   node packages/moss-agent/test/llm-usage.spec.mjs
 */
...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-llm-usage-test-'));
const origEnv = process.env.MOSS_LLM_USAGE_LOG;
const origWorkspaceDir = process.env.MOSS_WORKSPACE_DIR;
process.env.MOSS_LLM_USAGE_LOG = path.join(tmpDir, 'llm-usage.jsonl');

function cleanup() {
  fs.rmSync(tmpDir, { recursive: true, force: true });
  if (origEnv) {
    process.env.MOSS_LLM_USAGE_LOG = origEnv;
  } else {
    delete process.env.MOSS_LLM_USAGE_LOG;
  }
  if (origWorkspaceDir) {
    process.env.MOSS_WORKSPACE_DIR = origWorkspaceDir;
  } else {
    delete process.env.MOSS_WORKSPACE_DIR;
  }
}
...

{
  ...
  fs.unlinkSync(process.env.MOSS_LLM_USAGE_LOG);
...

{
  ...
  process.env.MOSS_LLM_USAGE_LOG = emptyPath;
...

{
  ...
  delete process.env.MOSS_LLM_USAGE_LOG;
  process.env.MOSS_WORKSPACE_DIR = workspace;
