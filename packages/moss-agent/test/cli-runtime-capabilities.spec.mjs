 ...
 *   node packages/moss-agent/test/cli-runtime-capabilities.spec.mjs
 */
...
test('CLI system prompt includes real runtime tools, project AGENTS, and honest CodeGraph status', async () => {
  ...
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-runtime-cwd-'));
  const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-runtime-config-'));
  try {
    fs.writeFileSync(
    ...
    assert.equal(
      result.status,
      0,
      `moss runtime capability smoke should exit cleanly\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`,
    );
...
test('CLI system prompt marks CodeGraph available only when registered MCP tools prove it', async () => {
  const mock = await startMockProvider();
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-runtime-codegraph-cwd-'));
  const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-cli-runtime-codegraph-config-'));
  try {
    fs.writeFileSync(
      path.join(configDir, 'community-auth.json'),
      JSON.stringify({
    ...
    const result = await runCli({
      ...
      extraEnv: {
        MOSS_MCP_ENABLED: '1',
        MOSS_MCP_CONFIG: mcpConfigPath,
      },
    });
    assert.equal(
      result.status,
      0,
      `moss CodeGraph runtime capability smoke should exit cleanly\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`,
    );
    assert.match(result.stdout, /runtime-ok/);
