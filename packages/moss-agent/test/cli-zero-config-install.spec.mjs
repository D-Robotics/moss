 ...
 *   node packages/moss-agent/test/cli-zero-config-install.spec.mjs
 */
...
if (process.platform !== 'win32') {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-zero-config-pack-'));
  execFileSync(process.execPath, [path.join(pkgRoot, 'scripts', 'prepare-zero-config-default.mjs'), '--package-dir', tmp], {
    env: {
      ...process.env,
      MOSS_ZERO_CONFIG_PROVIDER: 'openai-compatible',
      MOSS_ZERO_CONFIG_MODEL: 'Moss',
      MOSS_ZERO_CONFIG_BASE_URL: 'https://gateway.test/v1',
      MOSS_ZERO_CONFIG_API_KEY: 'public-token',
    },
    stdio: 'pipe',
  });
...
if (process.platform !== 'win32' && typeof process.getuid === 'function' && process.getuid() !== 0) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-zero-config-eacces-'));
  ...
  if (modeBitsEnforced) {
    ...
    let resolved;
    try {
      resolved = resolveCliConfig({ MOSS_BUNDLED_DEFAULT_FILE: file }, {});
    } finally {
...
{
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-zero-config-shadow-'));
  const file = path.join(tmp, 'zero-config-default.json');
  fs.writeFileSync(file, JSON.stringify({ provider: 'openai-compatible', model: 'M', baseUrl: 'https://g.test/v1', apiKey: 'k' }));
  // A half-filled config file (baseUrl, no key) shadows — and says so.
  const resolved = resolveCliConfig({ MOSS_BUNDLED_DEFAULT_FILE: file }, { baseUrl: 'https://corp.llm/v1' });
  ...
  const envLeftover = resolveCliConfig({ MOSS_BUNDLED_DEFAULT_FILE: file, OPENAI_BASE_URL: 'https://corp.llm/v1' }, {});
  ...
  const active = resolveCliConfig({ MOSS_BUNDLED_DEFAULT_FILE: file }, {});
