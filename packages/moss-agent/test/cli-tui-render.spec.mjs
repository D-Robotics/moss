 ...
 *   node packages/moss-agent/test/cli-tui-render.spec.mjs
 */
...
test('resolveTerminalThemeMode honors explicit and terminal background hints', () => {
  assert.equal(resolveTerminalThemeMode({ MOSS_TUI_THEME: 'light' }), 'light');
  assert.equal(resolveTerminalThemeMode({ MOSS_THEME: 'dark' }), 'dark');
...
test('board surface helpers distinguish SSH and local-board modes', () => {
  const previous = process.env.MOSS_BOARD_RUNTIME;
  delete process.env.MOSS_BOARD_RUNTIME;
  assert.equal(
  ...
  process.env.MOSS_BOARD_RUNTIME = '1';
  assert.equal(inferExecutionMode(), 'on-board');
  assert.equal(boardSurfaceLabel(), 'current machine is the board');
  if (previous === undefined) delete process.env.MOSS_BOARD_RUNTIME;
  else process.env.MOSS_BOARD_RUNTIME = previous;
});

test('executionPlaneSummary models PC Host, On-board, and Hybrid modes', () => {
  const previousBoard = process.env.MOSS_BOARD_RUNTIME;
  const previousHybrid = process.env.MOSS_HYBRID_MODE;
  delete process.env.MOSS_BOARD_RUNTIME;
  delete process.env.MOSS_HYBRID_MODE;
  ...
  process.env.MOSS_HYBRID_MODE = '1';
  ...
  if (previousBoard === undefined) delete process.env.MOSS_BOARD_RUNTIME;
  else process.env.MOSS_BOARD_RUNTIME = previousBoard;
  if (previousHybrid === undefined) delete process.env.MOSS_HYBRID_MODE;
  else process.env.MOSS_HYBRID_MODE = previousHybrid;
});
