 ...
 *   node packages/moss-agent/test/post-tool-source-fix.spec.mjs
 */
...
async function testMossAgentRetriesPartialStreamErrorWithoutPersistingPartial() {
  let calls = 0;
  const provider = new PiAiLLMProvider({
    apiKey: 'test-key',
    model: modelInfo(),
  ...
  console.log('  [PASS] MossAgent retries incomplete partial stream without persisting partial answer');
}
...
async function testLegacySessionMigrationPreservesThinking() {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'moss-session-legacy-'));
  try {
...
async function testMossAgentPersistsDoneMessageThinkingForToolUse() {
  const store = new InMemorySessionStore();
  const streamCalls = [];
  ...
  console.log('  [PASS] MossAgent persists done-message thinking on assistant tool_use turns');
}

async function testMossAgentKeepsEarlierToolThinkingAcrossSequentialTools() {
  const streamCalls = [];
  ...
  console.log('  [PASS] MossAgent keeps earlier assistant thinking across sequential tool calls');
}
...
await testMossAgentRetriesPartialStreamErrorWithoutPersistingPartial();
...
await testMossAgentPersistsDoneMessageThinkingForToolUse();
await testMossAgentKeepsEarlierToolThinkingAcrossSequentialTools();
