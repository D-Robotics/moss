 ...
 *   node packages/moss-agent/test/goal-command.spec.mjs
 */
...
assert.equal(
  JSON.stringify(storedAfterSet).includes('moss_goal_checkpoint'),
  true,
...
assert.doesNotMatch(requests[0].systemPrompt, /<moss_goal_mode>/);
assert.equal(
  JSON.stringify(requests[0].messages).includes('moss_goal_checkpoint'),
  false,
