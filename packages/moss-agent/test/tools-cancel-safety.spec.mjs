 ...
 *   node packages/moss-agent/test/tools-cancel-safety.spec.mjs
 */
...
async function assertShellLaunchedChildIsKilled(runShellCommand) {
  const tmp = await mkdtemp(path.join(os.tmpdir(), 'moss-run-process-'));
...
async function assertNodeLaunchedChildIsKilled(runLauncherCommand) {
  const tmp = await mkdtemp(path.join(os.tmpdir(), 'moss-run-process-tree-'));
  const pidFile = path.join(tmp, 'child.pid');
  let childPid = 0;
