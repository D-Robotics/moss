 ...
 *   node packages/moss-agent/test/redact.spec.mjs
 */
...
// ── MOSS_TELEMETRY_ALLOW parsing ──

{
  // Normal comma-separated list — sensitive fields are rejected
  const original = process.env.MOSS_TELEMETRY_ALLOW;
  process.env.MOSS_TELEMETRY_ALLOW = 'name,version,token,secret';
  const result = parseTelemetryAllow();
  assert.deepEqual(result, new Set(['name', 'version']));
  process.env.MOSS_TELEMETRY_ALLOW = original;
}

{
  // With whitespace — sensitive fields rejected, safe fields kept
  const original = process.env.MOSS_TELEMETRY_ALLOW;
  process.env.MOSS_TELEMETRY_ALLOW = ' model , password ';
  const result = parseTelemetryAllow();
...

{
  // Empty string
  const original = process.env.MOSS_TELEMETRY_ALLOW;
  process.env.MOSS_TELEMETRY_ALLOW = '';
  const result = parseTelemetryAllow();
...

{
  // Undefined
  const original = process.env.MOSS_TELEMETRY_ALLOW;
  delete process.env.MOSS_TELEMETRY_ALLOW;
  const result = parseTelemetryAllow();
  assert.deepEqual(result, new Set());
  process.env.MOSS_TELEMETRY_ALLOW = original;
}
...
{
  // MOSS_TELEMETRY_ALLOW integrates with redactSensitiveData
  // Sensitive fields in the env var are rejected; only safe fields pass through
  const original = process.env.MOSS_TELEMETRY_ALLOW;
  process.env.MOSS_TELEMETRY_ALLOW = 'model,prompt';
...
console.log('  [PASS] MOSS_TELEMETRY_ALLOW parsing and integration');
