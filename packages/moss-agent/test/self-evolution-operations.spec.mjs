...
const incomplete = trustedEnvironmentIdentity({
  workspaceDir: root,
  runtimeMode: 'device',
  device: { kernelVersion: '6.1' },
});
...
await fs.writeFile(
  path.join(root, '.moss', 'evolution.json'),
  JSON.stringify({
    experiment: {
      minSamplesPerArm: 4,
      wilsonZ: 2,
      maxCostRatio: 0.1,
      maxRetryIncrease: 'bad',
      unexpected: 9,
    },
  }),
  'utf8'
...
await experimentLog.append({
  ...
  control: { ...emptyArm, total: 0, passed: 0, successRate: 0 },
  treatment: emptyArm,
  sourceOutcomeIds: [],
});
...
console.log(
  'self-evolution-operations: identity, config, reports, read-only CLI and structured safety ok'
);
