...
if (process.env.MOSS_REALBOARD_TEST !== '1' || !HOST) {
  console.log(
    '  [SKIP] realboard-evolution-loop: set MOSS_REALBOARD_TEST=1 and MOSS_REALBOARD_HOST to opt in'
  );
  process.exit(0);
}
...
console.log(
  `  ✓ 真板环境身份完整:${realboardFacts.boardModel}, firmware=${realboardFacts.firmwareVersion ?? realboardFacts.kernelVersion}, fingerprint=${realboardIdentity.fingerprint}`
);
...
const created = await planTool.execute(
  {
    ...
    steps: [
      {
        description: 'yolov5 推理',
        expectedTools: ['device_exec'],
        expectedAccept: ['rdk-model-zoo'],
      },
    ],
    terminalAccept: [
...
assert.equal(
  terminal.verdict,
  'pass',
  `T3.3 终态应判 pass(真机 exit0 + bbox/score),实际=${terminal.verdict} reason=${terminal.reason}`
);
...
const recoveryCoordinator = new TrustedLearningCoordinator({
  eventLog: recoveryEventLog,
  memoryManager: recoveryMemory,
  patchCoordinator: recoveryPatchCoordinator,
});
...
await recoveryExperienceLog.append({
  schemaVersion: 2,
  id: 'controlled-fail-exp',
  sessionKey,
  taskId: plan.id,
  runId: recoveryRunId,
  attemptId: `${recoveryRunId}:controlled-fail`,
  stepId: `${plan.id}:step:1`,
  toolCallId: 'controlled-fail',
  evidenceId: 'controlled-fail',
  ...
  tool: 'device_exec',
  input: { command: 'test_yolov5.py (controlled timeout)' },
...
await recoveryExperienceLog.append({
  schemaVersion: 2,
  id: 'controlled-recover-exp',
  sessionKey,
  taskId: plan.id,
  runId: recoveryRunId,
  attemptId: `${recoveryRunId}:controlled-recover`,
  stepId: `${plan.id}:step:1`,
  toolCallId: 'controlled-recover',
  evidenceId: 'controlled-recover',
  contractSkill: 'rdk-model-zoo',
  contractVersion: '1.0.0',
  environmentFingerprint: env,
  environmentIdentityVersion: realboardIdentity.schemaVersion,
...
const recoveredGate = await recoveryGate({
  ...
  messages: [],
  totalToolCalls: 2,
  toolCallsByName: { device_exec: 2 },
  executionEvidence: {
    source: 'device_exec',
    toolUseId: 'controlled-recover',
    exitCode: sampleRun.exitCode,
    stdout: sampleRun.stdout,
    stderr: '',
  },
...
assert.deepEqual(
  recoveryEvents.map((event) => event.outcome),
  ['failed', 'recovered']
);
assert.equal(recoveryEvents[1].previousFailureId, recoveryEvents[0].id);
assert.ok(
  (await recoveryMemory.getAll()).some(
    (entry) => entry.trust === 'observation' && entry.topic?.includes('execution_failure')
  )
);
const [realboardPatch] = await recoveryPatchLog.latest();
assert.equal(
  realboardPatch.state,
  'proposed',
  '单次真板恢复只形成候选,未达到 2 个独立 proof 不自动发布'
);
await assert.rejects(() => fs.access(path.join(tmp, '.moss', 'skills', 'learned')));
assert.equal(
  (await recoveryExperimentLog.readAll()).length,
  0,
  '未发布的单次恢复候选不得进入 A/B 实验'
);
...
for (let i = 0; i < N; i++) {
  ...
  const gated = await wrapped({
    ...
    executionEvidence: {
      ...
      exitCode: runs[i].exitCode,
      stdout: runs[i].stdout,
      stderr: '',
    },
  });
...
).createTerminalStatsSource({ terminalVerdictLog: tvLog });
...
assert.equal(
  ...
  false,
  '跨信号未确认(X5 无 pose 对照 → 保守 false)'
);
assert.equal(
  records[0].decision.promotable,
  false,
  '★ 真机闭环:统计过但跨信号拒 → non-promotable(D6 切断,不自动升层)'
);
...
console.log(
  `  ✓ T3.4 闭环:统计过(statisticalPassed=true)+ 跨信号拒(crossSignalPassed=false)→ non-promotable → 1 Opinion 落盘`
);
