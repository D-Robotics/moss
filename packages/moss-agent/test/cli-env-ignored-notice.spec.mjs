 ...
 * `MOSS_LOG_LEVEL=warn` silence it. The doctor report keeps showing the
 ...
 *   node packages/moss-agent/test/cli-env-ignored-notice.spec.mjs
 */
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'moss-env-notice-'));
...
function run(args, extraEnv = {}) {
  ...
  const result = spawnSync(process.execPath, [distCli, ...args], {
    ...
    env: {
      ...
      MOSS_CONFIG_DIR: path.join(tmp, 'config'),
...
try {
  ...
  // MOSS_LOG_LEVEL=warn silences the notice.
  assert.doesNotMatch(
    run(['-p', 'hi'], { MOSS_LOG_LEVEL: 'warn' }),
    NOTICE,
    'MOSS_LOG_LEVEL=warn must silence the notice',
  );
