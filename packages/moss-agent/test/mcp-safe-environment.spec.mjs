...
console.log('[TEST] safeChildEnv strips host secrets');
{
  ...
  process.env.MOSS_API_KEY = 'secret-key-123';
  ...
  assert.equal(safeEnv.MOSS_API_KEY, undefined, 'MOSS_API_KEY should be stripped');
...
console.log('[TEST] MCP subprocess receives safe environment');
{
  ...
  const script = `
    console.log(JSON.stringify({
      MOSS_API_KEY: process.env.MOSS_API_KEY,
  ...
  assert.equal(env.MOSS_API_KEY, undefined, 'Subprocess should not receive MOSS_API_KEY');
