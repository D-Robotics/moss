#!/usr/bin/env node
/**
 * Self-test for digestToolCall.
 *
 * Run:
 *   npm run build -w @rdk-moss/agent/teaching
 *   node packages/moss-teaching/test/teaching-tool-digest.spec.mjs
 */

import assert from 'node:assert/strict';
import { digestToolCall } from '../dist/teaching/index.js';

// ── Deterministic output ──

{
  // Same inputs always produce the same digest
  const a = digestToolCall('shell_exec', { cmd: 'ls -la' });
  const b = digestToolCall('shell_exec', { cmd: 'ls -la' });
  assert.equal(a, b, 'identical inputs must produce identical digest');
}

{
  // Different tool names produce different digests
  const a = digestToolCall('shell_exec', { cmd: 'ls' });
  const b = digestToolCall('write_file', { cmd: 'ls' });
...

{
  ...
  const a = digestToolCall('shell_exec', { cmd: 'ls -la' });
  const b = digestToolCall('shell_exec', { cmd: 'pwd' });
...

{
  ...
  const a = digestToolCall('tool', { zebra: 1, alpha: 2, mango: 3 });
  const b = digestToolCall('tool', { mango: 3, zebra: 1, alpha: 2 });
...

{
  ...
  const a = digestToolCall('tool', { outer: { z: 1, a: 2 } });
  const b = digestToolCall('tool', { outer: { a: 2, z: 1 } });
  assert.equal(a, b, 'nested key order must not affect digest');
}

{
  // Different nested values produce different digests
  const a = digestToolCall('tool', { config: { mode: 'fast' } });
  const b = digestToolCall('tool', { config: { mode: 'slow' } });
...

{
  ...
  const a = digestToolCall('tool', { items: [1, 2, 3] });
  const b = digestToolCall('tool', { items: [1, 2, 3] });
...

{
  ...
  const a = digestToolCall('tool', { items: [1, 2, 3] });
  const b = digestToolCall('tool', { items: [3, 2, 1] });
...

{
  ...
  const a = digestToolCall('tool', null ?? {});
...

{
  ...
  const digest = digestToolCall('tool', { key: 'value' });
...

{
  ...
  for (let i = 0; i < 200; i++) {
    digests.add(digestToolCall('tool', { iteration: i, data: `sample-${i}` }));
  }
