 ...
 *   node packages/moss-agent/test/channel-bridge-timeout.spec.mjs
 */
...

{
  ...
  await assert.rejects(
    ...
    (err) => err instanceof MossError && err.code === ErrorCode.TOOL_EXECUTION_TIMEOUT,
  );
...

{
  ...
  await assert.rejects(
    ...
    (err) => err instanceof MossError && err.code === ErrorCode.TOOL_EXECUTION_FAILED,
  );
