 ...
 *   node packages/moss-agent/test/cli-args.spec.mjs
 */
...
{
  const workspaceArg = path.join(os.tmpdir(), 'moss-cli-args-workspace');
...
{
  const parsed = parseCliArgs(['--config-file', '/tmp/moss-custom.json', 'config', 'show']);
  assert.equal(parsed.command, 'config');
  assert.deepEqual(parsed.commandArgs, ['show']);
  assert.equal(parsed.prompt, '');
}
