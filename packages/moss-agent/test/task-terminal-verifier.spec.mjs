...
import {
  verifyTaskTerminal,
  arbitrateTaskTerminal,
} from '../dist/acceptance/task-terminal-verifier.js';
...
// ─── 4. 有 plan 无 terminalAccept → unknown ──────────────────────────────────
{
  const plan = {
    ...
    createdAt: '',
    updatedAt: '',
    successCriteria: ['model runs on board'],
...
// ─── 10. 冷启动 guard:样本不足 → driftChecks 空 ──────────────────────────────
{
  ...
  assert.ok(
    !arbitration.driftChecks || arbitration.driftChecks.length === 0,
    '样本不足(<10)→ 不跑漂移(冷启动 guard)'
  );
}
...
// ─── 11. 同一 execution 重试十次仍是冷启动 ────────────────────────────────────
{
  ...
  assert.deepEqual(
    retryDrift.arbitration.driftChecks,
    [],
    'one replayed execution cannot satisfy drift sample threshold'
  );
}
