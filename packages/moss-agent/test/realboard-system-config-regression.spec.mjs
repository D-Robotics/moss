...
if (process.env.MOSS_REALBOARD_TEST !== '1' || !HOST) {
  console.log('  [SKIP] realboard-system-config: set MOSS_REALBOARD_TEST=1 and MOSS_REALBOARD_HOST to opt in');
  process.exit(0);
}
...
assert.equal(stdoutVerdict.verdict, 'pass', `真机 nmcli 输出应判 pass(connected),实际=${stdoutVerdict.verdict} reason=${stdoutVerdict.reasonCode}`);
