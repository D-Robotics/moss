#!/usr/bin/env node
/**
 * Test: MCP process error handler wraps errors in MossError
 *
 * Verifies that when the child process emits an error (e.g., spawn failure),
 * pending requests are rejected with MossError, not raw Error.
 */

import assert from 'node:assert/strict';
import { connectMcpServersWithFailures } from '../dist/mcp/index.js';

// ── Real MCP server: process error is wrapped in MossError ──
