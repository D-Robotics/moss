#!/usr/bin/env node
/**
 * @rdk-moss/agent/memory — MemoryManager.buildDigest unit tests
 *
 * Run after package build:
 *   npm run build -w @rdk-moss/agent/memory && node packages/moss-memory/test/memory-digest.spec.mjs
 */
...
const distJs = path.join(dir, '..', 'dist', 'memory', 'index.js');
...
const base = await fs.mkdtemp(path.join(os.tmpdir(), 'moss-mem-digest-'));
...
try {
  ...
  // single entry → wrapped block with content, id, and usage guidance
  {
    ...
    assert.ok(d.includes('<moss_memory>') && d.includes('</moss_memory>'), 'wrapped in block');
