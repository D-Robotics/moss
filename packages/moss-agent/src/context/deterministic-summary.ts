...
function fallbackMessageNote(message: Message, index: number): string {
  ...
  for (const block of message.content) {
    ...
    if (block.type === 'tool_use') {
      parts.push(
        `tool_use ${block.name ?? 'tool'}(${sanitizeSecrets(stringifyToolInput(block.input))})`
      );
      continue;
    }
    if (block.type === 'tool_result') {
      ...
      const status = block.aborted?.by
        ...
        : block.is_error
          ? ' error'
          : '';
      parts.push(`tool_result ${block.name ?? 'tool'}${status}: ${JSON.stringify(body)}`);
    }
  }
...
function extractPrimaryUserGoal(messages: Message[]): string {
  for (const message of messages) {
    ...
    if (text.includes('<moss_working_context_checkpoint')) continue;
    if (text.includes('<moss_goal_checkpoint')) continue;
    return sanitizeSecrets(truncateMiddle(text, DETERMINISTIC_NOTE_MAX_CHARS));
  }
  return '未能从压缩窗口中可靠提取原始用户目标；继续前先核对当前任务。';
}

export function buildDeterministicCompactionSummary(messages: Message[], reason: string): string {
  ...
  const selected =
    messages.length <= firstCount + lastCount
      ...
      : [
          ...
          ...messages.slice(-lastCount).map((message, offset) => ({
            message,
