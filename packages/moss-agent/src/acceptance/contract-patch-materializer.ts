...
export class ContractPatchMaterializer {
  ...
  async publish(proposal: ContractPatchProposal, promotion: PromotionDecisionRecord): Promise<CandidatePatchRecord> {
    ...
    if (sourcePath && !allowedRoots.some((root) => sourcePath.startsWith(root))) errors.push('skill_not_workspace_owned');
    const contractPath = sourcePath ? path.join(path.dirname(sourcePath), 'ACCEPTANCE.json') : '';
    let raw: Record<string, unknown> | undefined;
    if (!errors.length) {
      try { raw = JSON.parse(await fs.readFile(contractPath, 'utf8')) as Record<string, unknown>; }
      catch { errors.push('acceptance_contract_unreadable'); }
    }
    if (raw && String(raw.version ?? '1') !== proposal.expectedVersion) errors.push('stale_contract_version');
    if (errors.length) return this.record(id, proposal, 'rejected', errors, 'contract_patch_rejected');

    await this.record(id, proposal, 'validated', [], 'whitelist_and_promotion_gates_passed');
    const section = Array.isArray(raw![proposal.section]) ? [...raw![proposal.section] as unknown[]] : [];
    if (proposal.index === undefined) section.push(proposal.spec);
    else if (proposal.index >= 0 && proposal.index < section.length) section[proposal.index] = proposal.spec;
    else return this.record(id, proposal, 'rejected', ['section_index_out_of_range'], 'contract_patch_rejected');
    const next = { ...raw!, [proposal.section]: section, version: nextVersion(proposal.expectedVersion) };
  ...
  async rollback(patchId: string): Promise<boolean> {
    ...
    if (!latest || latest.kind !== 'contract-params' || !latest.artifactPath || !latest.backupPath) return false;
    const paths = getMossWorkspacePaths(this.deps.workspaceDir);
    const allowedRoots = [paths.skillsDir, paths.agentSkillsDir].map((root) => `${path.resolve(root)}${path.sep}`);
  ...
  private async record(
    ...
    const record: CandidatePatchRecord = {
      schemaVersion: 1, id, revision: (previous?.revision ?? 0) + 1,
      kind: 'contract-params', state, skill: proposal.skill,
      environmentFingerprint: proposal.environmentFingerprint,
      failureClass: 'contract_drift', sourceEventIds: [...new Set(proposal.sourceEventIds)].sort(),
      toolSequences: [], reasonCode,
