...
export function buildStructuredOutputSystemPrompt(
  ...
  lines.push(
    '3. Call the tool ONCE (without `validateOnly`), then emit the JSON in a ```json block in your reply'
  );
  lines.push(
    '4. The agent loop validates your JSON against the schema automatically — if it fails, the errors are fed back to you as a correction and you get a retry (up to 3). You do NOT need to call the tool again with `validateOnly` to self-check.'
  );
  lines.push('');
  lines.push(
    '`validateOnly: true` is an OPTIONAL pre-check for JSON you already have on hand; it is not part of the normal flow.'
  );
  lines.push('');
