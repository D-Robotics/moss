...
export interface ExecuteToolCallDeps {
  ...
  /** Optional per-tool abort signal used by legacy MossAgent streamChat hosts. */
        ...
        (_, reject) =>
          (timeoutHandle = setTimeout(
            () => {
              ...
              reject(
                new MossError({ code: ErrorCode.TOOL_EXECUTION_TIMEOUT, message: `Tool ${call.name} timed out (${deps.toolTimeoutMs / 1000}s)` }),
              );
            },
            deps.toolTimeoutMs,
          )),
