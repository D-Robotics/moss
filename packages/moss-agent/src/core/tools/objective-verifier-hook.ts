...
const EXPLICIT_EXIT_SUFFIX_RE = /^(?:done|ok|success|completed)\s+\(exit\s+(-?\d+)\)\s*$/i;
const STANDALONE_EXIT_RE = /^\s*\(exit\s+(-?\d+)\)\s*$/i;
...
export function parseExitCode(result: string): number | null {
  for (const pattern of [EXEC_FAILED_RE, BACKGROUND_EXIT_RE, EXIT_CODE_LINE_RE, DEVICE_FAILED_RE, EXPLICIT_EXIT_SUFFIX_RE, STANDALONE_EXIT_RE]) {
