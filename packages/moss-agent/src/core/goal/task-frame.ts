...
const CHECKPOINT_START = '<moss_working_context_checkpoint';
const CHECKPOINT_END = '</moss_working_context_checkpoint>';
const MAX_TEXT = 900;
...
export function recordTaskFrameToolEnd(
  ...
  const hitGuard = /\[moss-agent\] Tool loop guard stopped/i.test(params.result);
  if (hitGuard) {
