...
function buildDraftMarkdown(evidence: SkillCandidateEvidence, score: SkillScoreResult): string {
  ...
  const triggerList = [evidence.candidateId, ...toolNames, '对话沉淀', 'conversation skill'].join(
    ','
  );
...
function buildTeachingSections(teachingMeta: SkillCandidateEvidence['teachingMeta']): string {
  if (
    !teachingMeta ||
    (!teachingMeta.preAnnotations?.length && !teachingMeta.postAnnotations?.length)
  ) {
    return '';
  }
  ...
  if (teachingMeta.postAnnotations?.length) {
    ...
    for (const post of teachingMeta.postAnnotations) {
      if (post.verifyHint) out += `- **验证方法**：${post.verifyHint}\n`;
      if (post.nextStepIfFails) out += `- **失败对策**：${post.nextStepIfFails}\n`;
      if (post.rollbackHint) out += `- **回滚说明**：${post.rollbackHint}\n`;
      if (post.confidence) out += `- **本步置信度**：${post.confidence}\n`;
    }
    out += '\n';
  }
