    ...
    for (const ann of this.annotations) {
      ...
      if (ann.phase === 'pre') {
        ...
        postAnnotations.push({
          ...
          confidenceReason:
            typeof patch.confidenceReason === 'string' ? patch.confidenceReason : undefined,
          nextStepIfFails:
            typeof patch.nextStepIfFails === 'string' ? patch.nextStepIfFails : undefined,
          rollbackSupported:
            typeof patch.rollbackSupported === 'boolean' ? patch.rollbackSupported : undefined,
          rollbackHint: typeof patch.rollbackHint === 'string' ? patch.rollbackHint : undefined,
          failureCard:
            rawCard && typeof rawCard === 'object'
              ? {
                  cause:
                    typeof (rawCard as Record<string, unknown>).cause === 'string'
                      ? ((rawCard as Record<string, unknown>).cause as string)
                      : undefined,
                  actions: Array.isArray((rawCard as Record<string, unknown>).actions)
                    ? ((rawCard as Record<string, unknown>).actions as string[])
                    : undefined,
                  stopWhen:
                    typeof (rawCard as Record<string, unknown>).stopWhen === 'string'
                      ? ((rawCard as Record<string, unknown>).stopWhen as string)
                      : undefined,
                  rollbackAvailable:
                    typeof (rawCard as Record<string, unknown>).rollbackAvailable === 'boolean'
                      ? ((rawCard as Record<string, unknown>).rollbackAvailable as boolean)
                      : undefined,
                }
              : undefined,
        });
