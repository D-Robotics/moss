 ...
 *   - Scope 前缀规范（[server] / [moss] / [agent] / [provider:pi-ai] 等）
...
function envLevel(): LogLevel | undefined {
  ...
  const raw = String(env.MOSS_LOG_LEVEL ?? '').trim().toLowerCase();
...
function envJson(): boolean {
  ...
  const env =
    (globalThis as { process?: { env?: Record<string, string | undefined> } }).process?.env ?? {};
  const raw = String(env.MOSS_LOG_JSON ?? '').trim().toLowerCase();
 ...
 *
 * const runLog = log.child('moss', { runId: 'a7f9' });
 * runLog.debug('拉取 skills', { count: 42 });
 * // 输出：[server:moss] [debug] 拉取 skills · runId=a7f9 · count=42
 * ```
 */
