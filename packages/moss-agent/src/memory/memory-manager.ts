...
const EN_DEVICE_PROJECT_ANCHOR =
  'device board model workspace project constraint devkit ip hostname';

export type MemorySource = 'memory' | 'sessions';

export type MemoryWriteValidation = { ok: true } | { ok: false; reason: string };
...
function looksLikeDeviceOrProjectRecallQuery(query: string): boolean {
  ...
  if (!q) return false;
  if (/[\u4e00-\u9fff]/.test(q)) {
    return /设备|板子|板卡|开发板|型号|工作区|项目|约束|网段|地址|hostname|上次|之前|记过|还记得|提到过/.test(
      q
    );
  }
  const lower = q.toLowerCase();
      ...
      .then(async () => {
        ...
        if (existingIndex >= 0) {
          ...
          if (options?.scopeRef !== undefined)
            this.entries[existingIndex].scopeRef = options.scopeRef;
  ...
  async buildDigest(options?: {
    ...
    const candidates = this.entries.filter((e) => {
      ...
      if (
        ...
        options?.projectHash &&
        e.scopeRef &&
        e.scopeRef !== options.projectHash
      ) {
        return false;
      }
      return true;
