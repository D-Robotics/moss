...
export class RecoveryRecipeLog {
  ...
  async append(recipe: RecoveryRecipe): Promise<boolean> {
    ...
    await this.chain.enqueue(this.filePath, async () => {
      ...
      if (existing.some((entry) => entry.id === recipe.id && entry.revision === recipe.revision && entry.state === recipe.state)) return;
...
function cameraTraceEligible(event: LearningEvent, experiences: ExperienceEntry[]): boolean {
  ...
  return experiences.some((entry) => {
    ...
    return /get_isp_data|ffmpeg|handle_.*\.yuv|photo.*\.jpe?g/i.test(command)
...
export function compileRecoveryRecipe(input: {
  ...
  const { event } = input;
  if (event.outcome !== 'recovered' || !event.skill || !event.failureClass || event.environmentFingerprint === 'unknown') return null;
  if (!cameraTraceEligible(event, input.experiences)) return null;
  const sourceEvents = [...input.relatedRecoveries, event]
    .filter((candidate, index, all) => all.findIndex((other) => other.id === candidate.id) === index);
  const sourceTaskRunIds = [...new Set(sourceEvents.map((candidate) => `${candidate.taskId}:${candidate.runId}`))].sort();
  const id = recoveryRecipeId(event);
  const immutable = input.previous?.state === 'published' || input.previous?.state === 'rolled_back';
  ...
  const outputCollision = input.experiences.some((entry) => (
  ...
  const verifiedBindings = {
    ...
    ...(Number.isInteger(width) && Number.isInteger(height) ? {
      width, height, frameBytes: Math.round(width * height * 1.5),
    } : {}),
  };
  const terminalAccept: AcceptSpec[] = [
    ...
    { name: 'image_dimensions', params: { path: artifactPath, width: '${width}', height: '${height}' } },
  ...
  return {
    ...
    failureSignature: {
      ...
      reasonCodes: [...new Set(sourceEvents.map((candidate) => candidate.reasonCode).filter(Boolean))].sort(),
    },
    ...
    steps: outputCollision ? [
      ...
      { tool: 'exec', operation: 'convert_to_unique_staging_jpeg', arguments: { input: '${sourceYuv}', output: '${stagingArtifactPath}', width: '${width}', height: '${height}' }, expectedEvidence: [{ name: 'image_content_nontrivial', params: { path: '${stagingArtifactPath}', minVariation: 50 } }] },
      { tool: 'exec', operation: 'promote_validated_artifact', arguments: { source: '${stagingArtifactPath}', output: artifactPath }, expectedEvidence: terminalAccept },
    ] : [
      { tool: 'exec', operation: 'mark_capture_boundary', arguments: { path: markerPath }, expectedEvidence: [{ name: 'file_exist', params: { path: markerPath } }] },
      { tool: 'exec', operation: 'probe_sensor_index', arguments: { source: 'get_isp_data_help' }, expectedEvidence: [{ name: 'stdout_matches', params: { pattern: 'sensor|index' } }] },
      { tool: 'exec', operation: 'capture_nv12_frame', arguments: { sensorIndex: '${sensorIndex}' }, expectedEvidence: [{ name: 'exit_code_zero', params: {} }] },
      { tool: 'exec', operation: 'select_fresh_size_valid_frame', arguments: { createdAfter: markerPath, expectedBytes: '${frameBytes}', output: '${sourceYuv}' }, expectedEvidence: [{ name: 'file_fresh_nonempty', params: { path: '${sourceYuv}', after: markerPath } }] },
      { tool: 'exec', operation: 'convert_nv12_to_jpeg', arguments: { input: '${sourceYuv}', output: artifactPath, width: '${width}', height: '${height}' }, expectedEvidence: terminalAccept },
    ],
    ...(outputCollision ? { executionMode: 'single-bounded-transaction' as const } : {}),
    terminalAccept,
    safetyConstraints: [{ name: 'stdout_matches', params: { pattern: '^(?!.*(?:reboot|shutdown|mkfs)).*$' }, safetyCritical: true }],
    bindings: {
      artifactPath: 'path', stagingArtifactPath: 'path', captureMarker: 'path', sourceYuv: 'path', sensorIndex: 'integer',
      width: 'integer', height: 'integer', frameBytes: 'bytes', runStartedAt: 'timestamp',
    },
    ...(Object.keys(verifiedBindings).length ? { verifiedBindings } : {}),
    invariants: outputCollision
      ? ['output-target-type', 'bounded-empty-collision-cleanup', 'unique-staging-output', 'validate-before-promote']
      : ['capture-boundary', 'fresh-source-frame', 'expected-frame-size', 'unique-output', 'decoded-image-content'],
    sourceEventIds: sourceEvents.map((candidate) => candidate.id).sort(),
    sourceTaskRunIds,
    sourceExperienceIds: [...new Set([...(input.previous?.sourceExperienceIds ?? []), ...input.experiences.map((entry) => entry.id)])].sort(),
...
export function validateRecoveryRecipe(recipe: RecoveryRecipe, baseSkillText = ''): RecoveryRecipeQualityReason {
  if (recipe.steps.length < 2 || recipe.steps.some((step) => !step.operation || step.expectedEvidence.length === 0)) {
    return 'insufficient_procedural_detail';
  }
  if (recipe.independentRecoveryCount < 2) return 'insufficient_independent_evidence';
  if (!recipe.environmentSelector.boardFamily || recipe.environmentSelector.fingerprint === 'unknown') {
    return 'overfit_to_single_environment';
  }
  if (Object.keys(recipe.bindings).length === 0 || recipe.steps.some((step) => JSON.stringify(step.arguments).includes('undefined'))) {
    return 'unsafe_or_unresolved_operation';
  }
  if (recipe.executionMode === 'single-bounded-transaction') {
    const allowed = new Set([
      'inspect_output_target_type', 'remove_exact_empty_output_collision',
      'convert_to_unique_staging_jpeg', 'promote_validated_artifact',
    ]);
  ...
  const hasSpecificCorrection = (recipe.invariants.includes('capture-boundary') && recipe.invariants.includes('fresh-source-frame'))
    || (recipe.invariants.includes('output-target-type') && recipe.invariants.includes('validate-before-promote'));
...
export function validateShadowReplay(input: {
  ...
  if (input.recipe.sourceTaskRunIds.includes(`${input.taskId}:${input.runId}`)
    || input.evidenceIds.some((id) => input.recipe.sourceExperienceIds.includes(id))) return 'shadow_evidence_overlap';
