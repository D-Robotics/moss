...
export async function readSelfEvolutionSnapshot(workspaceDir: string): Promise<SelfEvolutionSnapshot> {
  ...
  return { patches: await patchLog.latest(), records: await experimentLog.readAll() };
}
...
export function formatSelfEvolutionStatus(snapshot: SelfEvolutionSnapshot): string {
  ...
  return [
    ...
    `  experiments: assignments=${assignments.length}, exposed=${assignments.filter((entry) => entry.exposed).length}, outcomes=${outcomes.length}`,
...
export function formatSelfEvolutionExperiments(snapshot: SelfEvolutionSnapshot): string {
  ...
  for (const patch of snapshot.patches.sort((a, b) => a.id.localeCompare(b.id))) {
    ...
    lines.push(`  ${patch.id} skill=${patch.skill} patch=${patch.state} experiment=${decision?.state ?? 'not-started'} assignments=${assignments.length} exposed=${assignments.filter((entry) => entry.exposed).length} outcomes=${outcomes.length}`);
  }
  return lines.join('\n');
}
...
function armLine(label: string, arm: PatchExperimentDecision['control']): string {
  ...
  return `  ${label}: n=${arm.total}, success=${(arm.successRate * 100).toFixed(1)}% CI=[${(arm.wilsonLow * 100).toFixed(1)},${(arm.wilsonHigh * 100).toFixed(1)}], retries=${arm.averageRetries.toFixed(2)}, corrections=${arm.averageCorrections?.toFixed(2) ?? 'n/a'}, tools=${arm.averageToolCalls.toFixed(2)}, durationMs=${arm.averageDurationMs.toFixed(0)}, tokens=${(arm.averageInputTokens + arm.averageOutputTokens).toFixed(0)}, cost=${cost}, safety=${arm.safetyFailures}`;
}
