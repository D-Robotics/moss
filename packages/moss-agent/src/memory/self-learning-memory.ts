...
export function buildSelfLearningMemoryDraft(userMessage: string): SelfLearningMemoryDraft | null {
  ...
  const correction =
    /没改好|没有改好|不对|不太对|还是不行|不合理|很怪|奇怪|不好用|没法比|不够强|应该|以后|记住|记一下/.test(
      msg
    ) ||
    /\b(not right|still broken|doesn'?t work|does not work|not good enough|remember|next time|should|wrong|incorrect|fix this|that'?s not|please don'?t|stop doing|always use|never use|don'?t do|no,?\s+do|that'?s wrong|not what i|not correct)\b/.test(
      lower
    );
  if (!correction) return null;

  const productOrUx =
    /体验|产品|用户|文字|文案|入口|页面|工作台|对话|聊天|记忆|迭代|审批|按钮|折叠|刷新|图片|附件|plan|ux|copy|memory|agent|approval|button|chat|workspace/.test(
      lower
    ) ||
    /体验|产品|用户|文字|文案|入口|页面|工作台|对话|聊天|记忆|迭代|审批|按钮|折叠|刷新|图片|附件/.test(
      msg
    );
