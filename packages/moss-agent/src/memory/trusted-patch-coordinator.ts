...
export class TrustedPatchCoordinator {
  ...
  async observeLearningEvent(event: LearningEvent): Promise<CandidatePatchRecord | null> {
    ...
    if (event.failureClass === 'contract_drift' && event.outcome === 'failed') {
      return this.record(
        ...
        [event],
        [],
        'contract_requires_independent_review'
      );
    }
    ...
    const related = (await this.deps.eventLog.readAll())
      .filter(
        (candidate) =>
          candidate.outcome === 'recovered' &&
          (candidate.attribution === 'single-skill' ||
            candidate.attribution === 'single-owner-step') &&
      ...
      .filter(
        (candidate) =>
          !requiresRealDeviceEvidence(candidate.skill) || isRealEvidenceEligible(candidate)
      );
    const independentProofs = new Set(
      related.map((candidate) => `${candidate.taskId}:${candidate.runId}`)
    ).size;
    const sequences = uniqueSequences(related);
    const proposed = await this.record(
      event,
      'skill-guidance',
      'proposed',
      related,
      sequences,
      `recovery_proofs=${independentProofs}`
    );
    if (independentProofs < this.minRecoveryProofs) return proposed;
    const recipe =
      event.recoveryRecipeId && this.deps.recipeLog
        ? (await this.deps.recipeLog.latest(event.recoveryRecipeId))[0]
        : undefined;
    const recipeReason = recipe
    ...
    return this.record(
      ...
      related,
      sequences,
      'awaiting_held_out_shadow_replay'
    );
  }
  ...
  async observeShadowReplay(input: {
    ...
    if (!recipe || (recipe.state !== 'quality_validated' && recipe.state !== 'shadow_validated'))
      return null;
    const reason = validateShadowReplay({ recipe, ...input });
    const sourceEvents = (await this.deps.eventLog.readAll()).filter((event) =>
      recipe.sourceEventIds.includes(event.id)
    );
    ...
    if (reason !== 'quality_passed') {
      await this.deps.recipeLog.append({
        ...
        shadowEvidenceIds: [...input.evidenceIds].sort(),
        timestamp: new Date().toISOString(),
      });
      return this.record(
        event,
  ...
  private validateGuidance(event: LearningEvent, sequences: string[][]): string[] {
    ...
    if (
      sequences.some(
        (sequence) => sequence.length > 12 || sequence.some((tool) => !SAFE_TOOL.test(tool))
      )
    ) {
  ...
  private async publishGuidance(
    ...
    if (
      existingLatest?.state === 'published' &&
      JSON.stringify(existingLatest.sourceEventIds) === JSON.stringify(sourceEventIds)
    )
    ...
    const body = [
      ...
      ...(recipe
        ? [
            ...
            ...recipe.steps.map(
              (step, index) =>
                `${index + 1}. Use \`${step.tool}\` to \`${step.operation}\` with ${Object.entries(
                  step.arguments
                )
            ...
            ...(recipe.verifiedBindings && Object.keys(recipe.verifiedBindings).length
              ? [
                  `Verified bindings for this exact environment: ${Object.entries(
                    recipe.verifiedBindings
                  )
                    .map(([key, value]) => `\`${key}=${String(value)}\``)
                    .join(', ')}.`,
    ...
    try {
        ...
        `${JSON.stringify(
          {
            ...
            recoveryRecipeId: recipe?.id,
            recoveryRecipeRevision: recipe?.revision,
      ...
      published = await this.record(
        ...
        related,
        sequences,
        'auto_published_trusted_recovery',
        [],
        targetPath,
        backupPath,
        true
  ...
  private async validateRecipe(
    recipe: RecoveryRecipe
  ): Promise<ReturnType<typeof validateRecoveryRecipe>> {
    const registry =
      this.deps.skillRegistry ?? new SkillRegistry({ workspaceDir: this.deps.workspaceDir });
    const meta = registry
      .list()
      .find((skill) => skill.name === recipe.skill && !/trusted-recovery/i.test(skill.sourcePath));
    let baseSkillText = '';
    if (meta?.sourcePath) {
      try {
        baseSkillText = await fs.readFile(meta.sourcePath, 'utf8');
      } catch {
        baseSkillText = '';
      }
    }
  ...
  async rollback(patchIdValue: string): Promise<boolean> {
    const history = (await this.deps.patchLog.readAll()).filter(
      (record) => record.id === patchIdValue
    );
    ...
    if (!artifactPath.startsWith(learnedRoot) || path.basename(artifactPath) !== TRUSTED_SKILL_FILE)
      return false;
    const artifactDir = path.dirname(artifactPath);
    if (latest.backupPath) {
      const backupPath = path.resolve(latest.backupPath);
      if (
        path.dirname(backupPath) !== artifactDir ||
        !TRUSTED_SKILL_BACKUP_RE.test(path.basename(backupPath))
      )
        return false;
  ...
  private async record(
    ...
    const record: CandidatePatchRecord = {
      ...
      sourceEventIds: sources.map((entry) => entry.id).sort(),
      toolSequences: sequences,
      reasonCode,
