...
import type { MossAgent, MossAgentEvent, GoalState, Tool, ToolResultOutcome } from '../core/index.js';
...
function activityLabel(event: MossAgentEvent): string | null {
...
export function MossTui({ agent, skillLearner, runtime, sessionKey: initialSessionKey }: MossTuiProps): React.ReactElement {
  ...
  const [showThinking, setShowThinking] = useState(process.env.MOSS_SHOW_THINKING === 'true');
