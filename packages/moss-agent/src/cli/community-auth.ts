...
export interface MossCommunityAuthLoginOptions {
  manual?: boolean;
  openBrowser?: boolean;
...
export class MossCommunityAuthRequiredError extends Error {
  constructor(message = renderCommunityAuthRequiredMessage()) {
    super(message);
    this.name = 'MossCommunityAuthRequiredError';
  }
}
...
function normalizeSsoBaseUrl(env: NodeJS.ProcessEnv = process.env): string {
  const raw = env.MOSS_COMMUNITY_SSO_BASE_URL || env.MOSS_SSO_BASE_URL || DEFAULT_SSO_BASE_URL;
...
function userFromClaims(claims: Record<string, unknown> | null): MossCommunityUser | null {
...
function userFromPortalTokenDigest(accessToken: string): MossCommunityUser {
...
function parseStoredSession(raw: unknown): MossCommunityAuthSession | null {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;
  const value = raw as Partial<MossCommunityAuthSession>;
  ...
  return value as MossCommunityAuthSession;
}
...
export function getMossCommunityAuthStatus(options: {
  ...
  const session = readMossCommunityAuthSession(configDir);
  if (!session) {
...
export function clearMossCommunityAuthSession(configDir = resolveConfigDir()): boolean {
  const sessionPath = resolveCommunityAuthSessionPath(configDir);
  try {
...
export async function runMossCommunityAuthLogin(options: {
  ...
  try {
    ...
    const now = Date.now();
    const session: MossCommunityAuthSession = {
      schema: AUTH_SCHEMA,
      ssoBaseUrl,
      accessToken: token,
      user: verified.user,
    ...
    const sessionPath = writeMossCommunityAuthSession(session, configDir);
...
export async function ensureMossCommunityAuth(options: {
  ...
  if (!options.interactive) {
    throw new MossCommunityAuthRequiredError();
  }

  return runMossCommunityAuthLogin({
    configDir,
    env,
    fetchImpl: options.fetchImpl,
    openBrowser: true,
