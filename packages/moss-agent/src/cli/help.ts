...
export function displayHelp(c: Colors, options: { all?: boolean } = {}): void {
  ...
  const lines = [
    ...
    `    ${c.green('plugins list')}          list explicitly installed trusted plugins`,
    `    ${c.green('plugins add')} ${c.dim('<path-or-package>')} install a local or npm Moss plugin`,
    `    ${c.green('plugins doctor')}        validate installed plugin manifests and runtime exports`,
    ...
    `    ${c.magenta('MOSS_DEVICE_HOST')}       ${c.dim('default device IP/hostname for /connect')}`,
    ...
    `    ${c.magenta('MOSS_DEVICE_NO_VERIFY')}  ${c.dim('=1 → make /connect skip its hostname probe')}`,
    `    ${c.magenta('MOSS_DEVICE_HYBRID')}     ${c.dim('=1 → make /connect keep local tools (hybrid mode)')}`,
