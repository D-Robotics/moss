...
export function createMemoryTools(memoryManager: MemoryManager): Tool[] {
  const memoryRead: Tool = {
    name: 'memory_read',
    description:
      'Search long-term memory (persists across sessions) for relevant entries — user preferences, past decisions, project/device facts, prior solutions. The <moss_memory> session digest is only an overview; use this to retrieve specifics. Returns each match with its id.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  const memoryWrite: Tool = {
    name: 'memory_write',
    description:
      'Save a durable fact for future sessions — a user preference, key decision, project/device constraint, or hard-won solution. One fact per call; check the <moss_memory> digest first to avoid duplicates. Not for transient details or secrets.',
    inputSchema: {
      type: 'object',
      properties: {
