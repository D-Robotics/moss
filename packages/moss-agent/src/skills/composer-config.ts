import type { SkillComposerConfig, SkillComposerProviderMode } from './composer-types.js';
...
function parseMode(value: unknown, fallback: SkillComposerProviderMode): SkillComposerProviderMode {
  ...
  if (typeof value !== 'string' || !PROVIDER_MODES.has(value as SkillComposerProviderMode)) {
    throw new Error(
      'skills.composer.mode must be legacy, rules, local-model, remote-model, or auto'
    );
  }
...
export function normalizeSkillComposerConfig(
  ...
  const shadowProvider =
    shadowProviderRaw === undefined ? undefined : parseMode(shadowProviderRaw, 'rules');
  ...
  return {
    ...
    minConfidence: boundedNumber(input?.minConfidence, 0, 0, 1, 'skills.composer.minConfidence'),
