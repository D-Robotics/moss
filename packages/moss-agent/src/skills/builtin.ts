...
- No error handling for impossible states.`,
  },
  {
    ...
    trigger: ['bug', 'failure', 'regression', 'unexpected behavior', 'systematic-debugging', '调试', '除零', '根因', 'bug 修复'],
    risk: 'low',
    permissions: {},
    runtimePolicy: { delegatePreference: 'hybrid', approvalLevel: 'none' },
    enabled: true,
...
- Don't add error handling to mask the bug — fix the bug.`,
  },
  {
    ...
    trigger: ['tdd', 'test first', 'failing test', 'behavior change', 'bug fix', '测试驱动', '先写测试', '失败测试'],
    risk: 'low',
    permissions: {},
...
- Don't migrate lazily without a fallback when the new path is missing.`,
  },
  {
    ...
    trigger: ['codegraph', 'callers', 'callees', 'trace', 'impact radius', 'where is defined', '调用者', '调用关系', '影响面'],
    risk: 'low',
...
- After edits, run \`codegraph sync <repo>\` so the index catches up.`,
  },
  {
    ...
    trigger: ['code review', 'review this', 'review the code', 'audit', 'check for bugs', '审查', '代码审查', '检查 bug', 'review'],
    risk: 'low',
    permissions: { workspaceRead: true },
    runtimePolicy: { delegatePreference: 'hybrid', approvalLevel: 'none' },
    enabled: true,
...
Return findings ranked by severity, critical first. If there are no issues, say so plainly — do not invent nits to seem thorough.`,
  },
  {
    ...
    trigger: ['git', 'commit', 'branch', 'merge', 'rebase', 'stash', 'diff', 'pull request', '提交', '分支', '合并'],
    risk: 'medium',
    permissions: { workspaceRead: true, workspaceWrite: true },
...
- Title matches the first commit. Body: what changed, why, how it was verified, migration notes.`,
  },
  {
    ...
    trigger: ['refactor', 'clean up', 'simplify', 'extract method', 'rename', 'code smell', '重构', '代码异味', '小步改'],
    risk: 'low',
    permissions: { workspaceRead: true, workspaceWrite: true },
    runtimePolicy: { delegatePreference: 'local', approvalLevel: 'none' },
    enabled: true,
