...
export function hashSystemPromptForTelemetry(
  combined: string,
  layerIds: string[]
): {
  ...
  const combinedHashShort = crypto
    .createHash('sha256')
...
export function hashSystemPromptLayers(
  ...
  for (const l of layers) {
    ...
    layerHashes[l.id] = crypto
      .createHash('sha256')
      .update(l.content, 'utf8')
      .digest('hex')
      .slice(0, 12);
  }
  return { combinedHashShort, combinedLength: combined.length, layerHashes };
...
export function hashStableDynamicSystemPrompt(
  stablePrefix: string,
  ...
  const stableHashShort = crypto
    .createHash('sha256')
    .update(stablePrefix, 'utf8')
    .digest('hex')
    .slice(0, 12);
  const dynamicHashShort = crypto
    .createHash('sha256')
    .update(dynamicSuffix, 'utf8')
    .digest('hex')
    .slice(0, 12);
