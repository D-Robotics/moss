...
const BROADCAST_PORT = parseInt(process.env.MOSS_MESH_DISCOVERY_PORT || '9091', 10) || 9091;
...
export class LanDiscovery {
  ...
  constructor(config: LanDiscoveryConfig) {
    this.config = config;
    this.peerTtlMs = Number(process.env.MOSS_MESH_PEER_TTL_MS) || 5 * 60 * 1000;
  }
  ...
  async start(): Promise<void> {
    ...
    this.socket.on('message', (data, rinfo) => {
      try {
        ...
        if (msg.type !== 'moss-announce') return;
