...
const DEFAULT_LEGACY_REGISTRY = 'C:/Users/tongchun.zhao/nodejs-v22/node-v22.23.1-win-x64/node_modules/@rdk-moss/agent/dist/skills/registry.js';

const RDK_TASKS = [
  { id: 'RDK-01', kind: 'single', lang: 'zh', prompt: '介绍一下 RDK X5 开发板的硬件能力和板卡规格。', expectedSkills: ['rdk-hardware'] },
  { id: 'RDK-02', kind: 'single', lang: 'en', prompt: 'Connect to the attached RDK board, identify the board model, and inspect its runtime status.', expectedSkills: ['rdk-board-knowledge'] },
  { id: 'RDK-03', kind: 'multi', lang: 'zh', prompt: '先识别并连接这块 RDK 开发板，然后检查 ROS 2 节点和话题是否正常。', expectedSkills: ['rdk-board-knowledge', 'rdk-device', 'rdk-ros'] },
  { id: 'RDK-04', kind: 'single', lang: 'en', prompt: 'Find the official RDK documentation for deploying an LLM model and summarize the supported workflow.', expectedSkills: ['rdk-doc-finder'] },
  { id: 'RDK-05', kind: 'single', lang: 'zh', prompt: '用已连接的 RDK 摄像头拍一张照片并保存到工作目录。', expectedSkills: ['rdk-capture-photo'] },
...
function toolCalls(events) {
  return events.flatMap((event) => event.type === 'assistant' && Array.isArray(event.message?.content)
...
async function main() {
  ...
  const allTasks = [...SKILL_TASKS.map((task) => ({ ...task, suite: 'historical' })), ...RDK_TASKS.map((task) => ({ ...task, suite: 'rdk-replay' }))];
  const { SkillRegistry } = await import(pathToFileURL(path.resolve(options.legacyRegistry)).href);
  const registry = new SkillRegistry({ workspaceDir: path.resolve('D:/moss-eval/fixtures/skill-eval/sample-lib') });
  const taskRows = [];
  ...
  for (const task of allTasks) {
    ...
    const namedLoads = loads.map((call) => call.input?.name).filter((name) => typeof name === 'string');
    let matched = [];
    const start = process.hrtime.bigint();
    for (let iteration = 0; iteration < 100; iteration++) matched = registry.matchByText(task.prompt);
    ...
    taskRows.push({
      ...
      injectedCharsEstimate: matched.reduce((sum, skill) => sum + (skill.body?.length ?? skill.description?.length ?? 0), 0),
  ...
  const result = {
    ...
    sourceDigest: crypto.createHash('sha256').update(JSON.stringify(taskRows.map((row) => ({ id: row.id, terminalReason: row.terminalReason, durationMs: row.durationMs })))).digest('hex'),
    caveats: [
    ...
    metrics: {
      ...
      singleExactMatch: accuracy(groups((row) => row.suite === 'historical' && row.kind === 'single')),
      multiExactMatch: accuracy(groups((row) => row.suite === 'historical' && row.kind === 'multi')),
      rejectionAccuracy: accuracy(groups((row) => row.suite === 'historical' && row.kind === 'reject')),
      chineseExactMatch: accuracy(groups((row) => row.suite === 'historical' && row.language === 'zh')),
      englishExactMatch: accuracy(groups((row) => row.suite === 'historical' && row.language === 'en')),
      rdkExactMatch: accuracy(groups((row) => row.suite === 'rdk-replay')),
      averageMatcherLatencyMs: mean(matchLatencies),
      p95MatcherLatencyMs: [...matchLatencies].sort((a, b) => a - b)[Math.floor(matchLatencies.length * 0.95)] ?? 0,
      ...
      downstreamProxyPassRate: mean(taskRows.filter((row) => row.downstreamProxyPassed !== undefined).map((row) => row.downstreamProxyPassed ? 1 : 0)),
      averageEndToEndDurationMs: mean(taskRows.map((row) => row.durationMs).filter((value) => typeof value === 'number')),
      terminalReasons: Object.fromEntries([...new Set(taskRows.map((row) => row.terminalReason))].map((reason) => [reason, taskRows.filter((row) => row.terminalReason === reason).length])),
    },
    tasks: taskRows,
  };
  await fs.mkdir(options.output, { recursive: true });
  await fs.writeFile(path.join(options.output, 'baseline.json'), JSON.stringify(result, null, 2));
  await fs.writeFile(path.join(options.output, 'summary.md'), `# Legacy skill baseline\n\nGenerated: ${result.generatedAt}\n\nSource: \`${result.sourceRun}\`\n\n\`downstreamProxyPassed\` is historical process completion, not a persisted semantic verifier result.\n\n\`\`\`json\n${JSON.stringify(result.metrics, null, 2)}\n\`\`\`\n`);
