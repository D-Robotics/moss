...
const requestedCostMetrics = (process.env.MOSS_AGENT_AB_COST_METRICS ?? '')
  ...
  .filter((value) => allowedCostMetrics.has(value));
const experimentHypothesis =
  process.env.MOSS_AGENT_AB_HYPOTHESIS === 'success_superiority'
    ? 'success_superiority'
    : scenario === 'camera'
...
const experimentCostMetrics = requestedCostMetrics.length
  ...
  : scenario === 'camera'
    ? ['durationMs']
    : ['retries', 'toolCalls', 'durationMs', 'tokens'];
...
const experimentThresholds = {
  ...
  successNoninferiorityMargin: finiteEnvNumber(
    'MOSS_AGENT_AB_SUCCESS_NONINFERIORITY_MARGIN',
    0.05,
    0,
    0.5
  ),
  minCostImprovementRatio: finiteEnvNumber('MOSS_AGENT_AB_MIN_COST_IMPROVEMENT_RATIO', 0.1, 0, 1),
  minCostMetricsImproved: Math.max(
    1,
    Math.min(
      4,
      Number.parseInt(
        process.env.MOSS_AGENT_AB_MIN_COST_METRICS_IMPROVED ?? (scenario === 'camera' ? '1' : '2'),
        10
      )
    )
  ),
...
const requestedConcurrency = Math.max(
  1,
  Math.min(4, Number.parseInt(process.env.MOSS_AGENT_AB_CONCURRENCY ?? '4', 10))
);
...
const maxLaunches = Math.max(
  targetPerArm * 2,
  Number.parseInt(process.env.MOSS_AGENT_AB_MAX_LAUNCHES ?? String(targetPerArm * 4), 10)
);
const benchmarkDir = path.resolve(
  process.env.MOSS_AGENT_AB_WORKSPACE ?? path.join(os.tmpdir(), `moss-${skillName}-real-agent-ab`)
);
...
await fs.writeFile(
  path.join(benchmarkDir, '.moss', 'evolution.json'),
  `${JSON.stringify(
    {
      experiment: {
...
try {
  ...
  identity = trustedEnvironmentIdentity({
  ...
  process.stdout.write(
    `[bootstrap] board=${facts.boardModel ?? 'unknown'} fingerprint=${identity.fingerprint}\n`
  );
} catch (error) {
  await ssh.close();
  throw error;
...
const experimentCoordinator = new TrustedSkillExperimentCoordinator({
  ...
  readUsage: () =>
    readUsageLog({ logPath: resolveLLMUsageLogPath({ workspaceDir: benchmarkDir }) }),
});
...
async function ensurePublishedPatch() {
  const existing = (await patchLog.latest()).find(
    (record) =>
  ...
  for (let index = 1; index <= 2; index += 1) {
    ...
    if (scenario === 'camera') {
      ...
      const captureCommand =
        "(sleep 8; printf 'lq') | timeout 30 /app/multimedia_samples/sample_isp/get_isp_data/get_isp_data -s 50 -c io >/dev/null 2>&1";
      ...
      await experienceLog.append({
        schemaVersion: 2,
        id: `bootstrap-capture-experience-${index}`,
        sessionKey,
        taskId: plan.id,
        runId,
        attemptId: `${runId}:${captureEvidence}`,
        stepId: `${plan.id}:step:1`,
        toolCallId: captureEvidence,
        evidenceId: captureEvidence,
      ...
      if (capture.exitCode !== 0)
        throw new Error(`bootstrap_camera_capture_failed:${capture.exitCode}`);
      const yuv = (
        await ssh.run('ls -t /root/handle_*.yuv | head -1', { timeout: 15_000 })
      ).stdout.trim();
      if (!/1920x1080.*\.yuv$/.test(yuv)) throw new Error(`bootstrap_camera_yuv_missing:${yuv}`);
      await ssh.run(
        "sh -c 'if [ -d /tmp/photo.jpg ]; then rmdir /tmp/photo.jpg; fi; rm -f /tmp/photo.jpg; mkdir /tmp/photo.jpg'",
        { timeout: 15_000 }
      );
    ...
    await experienceLog.append({
      ...
      tool: 'exec',
      input: {
        command: scenario === 'camera' ? failedCommand : '<redacted-controlled-readonly-failure>',
      },
    ...
    if (
      ...
      (scenario === 'hardware' && !/aarch64/i.test(recovered.stdout))
    ) {
    ...
    if (!accepted.ok)
      throw new Error(`bootstrap_recovery_was_not_accepted:${accepted.reason ?? 'unknown'}`);
  }

  if (scenario !== 'camera') {
    throw new Error('actionable_recipe_adapter_unavailable:run_with_MOSS_AGENT_AB_SCENARIO=camera');
  }
  const recipe = (await recipeLog.latest()).find(
    (entry) =>
  ...
  const shadowCreated = await planTool.execute(
    {
      action: 'create',
      goal: 'Held-out RDK X5 camera output-collision Shadow replay',
      steps: [
        {
          description: 'Recover a fresh JPEG from an occupied output path',
          expectedTools: ['exec'],
          expectedAccept: [skillName],
        },
      ],
  ...
  if (!/1920x1080.*\.yuv$/.test(shadowYuv))
    throw new Error(`shadow_camera_yuv_missing:${shadowYuv}`);
  await ssh.run(
    "sh -c 'if [ -d /tmp/photo.jpg ]; then rmdir /tmp/photo.jpg; fi; rm -f /tmp/photo.jpg; mkdir /tmp/photo.jpg'",
    { timeout: 15_000 }
  );
  ...
  await experienceLog.append({
    schemaVersion: 2,
    id: `shadow-experience-${shadowEvidence}`,
    sessionKey: shadowSessionKey,
    taskId: shadowPlan.id,
    runId: shadowRunId,
    attemptId: `${shadowRunId}:${shadowEvidence}`,
    stepId: `${shadowPlan.id}:step:1`,
    toolCallId: shadowEvidence,
    evidenceId: shadowEvidence,
    contractSkill: skillName,
    contractVersion: '1',
    environmentFingerprint: identity.fingerprint,
    environmentIdentityVersion: identity.schemaVersion,
  ...
  const shadowAccepted = await terminalGate({
    ...
    runId: shadowRunId,
    turn: 1,
    response: 'held-out shadow verified',
    messages: [],
    totalToolCalls: 1,
    toolCallsByName: { exec: 1 },
    executionEvidence: {
      source: 'exec',
      toolUseId: shadowEvidence,
      exitCode: shadowResult.exitCode,
      stdout: shadowResult.stdout,
      stderr: shadowResult.stderr,
    },
  });
  if (!shadowAccepted.ok)
    throw new Error(`shadow_replay_terminal_failed:${shadowAccepted.reason ?? 'unknown'}`);
  const shadowPublished = await patchCoordinator.observeShadowReplay({
    recipeId: recipe.id,
    taskId: shadowPlan.id,
    runId: shadowRunId,
    evidenceIds: [shadowEvidence],
    verdict: 'pass',
  });
  if (!shadowPublished || shadowPublished.state !== 'published')
    throw new Error('shadow_replay_did_not_publish_patch');

  const published = (await patchLog.latest()).find(
    (record) =>
      record.skill === skillName &&
      record.environmentFingerprint === identity.fingerprint &&
      record.state === 'published'
...
async function reconcileProcessResult(result) {
  ...
  const record = {
    ...
    ...(result.processError
      ? {
          reasonCode: result.processError.startsWith('agent_timeout:')
...
async function reconcileRecordedProcessFailures() {
  ...
  for (const record of recorded) {
    if (!record.failed || record.reasonCode === 'agent_process_interrupted_for_integrity_fix')
      continue;
    await experimentCoordinator.recordRunProcessFailure({
      patchId: patch.id,
      runId: record.runId,
...
process.stdout.write(
  `[resume] eligible control=${counts.control}/${targetPerArm} treatment=${counts.treatment}/${targetPerArm} priorLaunches=${launches}\n`
);

while (
  (counts.control < targetPerArm || counts.treatment < targetPerArm) &&
  ...
  const desiredVariant =
    counts.control >= targetPerArm && counts.treatment < targetPerArm
      ...
      : counts.treatment >= targetPerArm && counts.control < targetPerArm
        ? 'control'
        : counts.control <= counts.treatment
          ? 'control'
          : 'treatment';
  const batch = Array.from({ length: batchSize }, (_, offset) =>
    launchAgent(launches + offset + 1, desiredVariant)
  );
  ...
  const failures =
    processResults.filter((entry) => entry.failed).length +
    settled.filter((entry) => entry.status === 'rejected').length;
  process.stdout.write(
    `[progress] launches=${launches}/${maxLaunches} control=${counts.control}/${targetPerArm} treatment=${counts.treatment}/${targetPerArm} processFailures=${failures}\n`
  );
}

const records = await experimentLog.readAll();
const decisions = records.filter(
  (record) => record.kind === 'decision' && record.patchId === patch.id
);
...
const summary = {
  ...
  processResults: processResults.map(({ runId, exitCode, failed, reasonCode }) => ({
    runId,
    exitCode,
    failed,
...
if (scenario === 'camera') {
  try {
    await ssh.run(
      "sh -c 'if [ -d /tmp/photo.jpg ]; then rmdir /tmp/photo.jpg; fi; rm -f /tmp/moss-shadow-*.jpg'",
      { timeout: 15_000 }
    );
  } catch {
