...
export function buildAgentBehaviorPromptQuick(): string {
  return [
    ...
    '- Long-running processes: `exec` with `run_in_background: true` (or `exec_background`) for servers/watchers; read output with `exec_logs`, stop with `exec_stop`. Do not block a foreground `exec` on a process that never exits.',
    ...
    '- For multi-step tasks (3+ concrete steps), call `todo_write` at the start with the full plan, keep exactly one item `in_progress`, and update the list as steps complete so you do not lose the thread.',
