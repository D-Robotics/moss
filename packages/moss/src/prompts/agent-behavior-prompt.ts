...
export function buildAgentBehaviorPromptQuick(): string {
  return [
    ...
    '- Network → `web_search` for breadth (title/url/snippet), then `web_fetch` for depth on chosen URLs. For multi-angle comparisons use `query_keyword_groups` (up to 5) in one `web_search` call instead of serial searches. On `web_fetch`, pass `focus` with keywords when you need a topical extract from a long page. Prefer one parallel batch of independent tool lookups.',
