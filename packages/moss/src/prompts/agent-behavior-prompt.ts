...
export function buildAgentBehaviorPromptQuick(): string {
  return [
    ...
    '',
    '# Using your tools',
    '- Prefer specialized tools over general ones (e.g. `read_file` over `exec + cat`, `edit_file`/`multi_edit` over `exec + sed`/`python -c`, `run_tests`/`verify_fix` over ad-hoc test scripts). Prefer `edit_file` for one surgical edit; use `multi_edit` for 2+ known replacements; use `write_file` only for new files or full rewrites.',
