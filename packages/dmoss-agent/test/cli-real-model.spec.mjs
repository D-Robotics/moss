 ...
 *   node packages/dmoss-agent/test/cli-real-model.spec.mjs
 */
...
import { buildDmossCliIdentity } from '../dist/cli/identity.js';

function freshConfigDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'dmoss-realmodel-'));
  return { DMOSS_CONFIG_DIR: dir };
}
...
test('current_model tool reports the real backing model', async () => {
  ...
  process.env.DMOSS_CONFIG_DIR = env.DMOSS_CONFIG_DIR;
  try {
    ...
    delete process.env.DMOSS_CONFIG_DIR;
  }
});
...
test('identity guides the agent to the current_model tool only on the bundled gateway', () => {
  ...
  const userModel = buildDmossCliIdentity({ model: 'gpt-4o' });
