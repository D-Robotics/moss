 ...
 * existing ~/.config/dmoss is kept until ~/.config/moss is created, so upgrading
 * never orphans a user's config. Explicit DMOSS_CONFIG_DIR (also reachable as
 * MOSS_CONFIG_DIR via the env-alias bridge) still overrides everything.
 *
...
try {
  ...
  assert.equal(
    resolveConfigDir({ DMOSS_CONFIG_DIR: path.join(tmp, 'explicit') }),
    path.join(tmp, 'explicit'),
    'explicit DMOSS_CONFIG_DIR is honored verbatim',
  );
  ...
  if (process.platform !== 'win32') {
    ...
    assert.equal(resolveConfigDir({ XDG_CONFIG_HOME: xdg }), path.join(xdg, 'dmoss'), 'legacy dmoss kept when moss absent');
