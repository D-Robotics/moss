...
export function displayHelp(c: Colors, options: { all?: boolean } = {}): void {
  ...
  const lines = [
    ...
    `    ${c.magenta('DMOSS_IMAGE_INPUT')}       ${c.dim('=false to disable image input (on by default for all providers)')}`,
    ...
    `    ${c.magenta('DMOSS_DEVICE_NO_VERIFY')}  ${c.dim('=1 → skip the startup SSH reachability probe')}`,
    `    ${c.magenta('DMOSS_DEVICE_HYBRID')}     ${c.dim('=1 → keep local tools at startup instead of board mode')}`,
    `    ${c.magenta('DMOSS_MESH_ENABLED')}      ${c.dim('=true → start the agent mesh (DMOSS_MESH_PORT/_PEERS/_SHARED_SECRET)')}`,
    `    ${c.magenta('DMOSS_MCP_ENABLED')}       ${c.dim('=true/false → override MCP enablement from config')}`,
    `    ${c.magenta('DMOSS_MCP_CONFIG')}        ${c.dim('path to MCP servers JSON (overrides config dir)')}`,
