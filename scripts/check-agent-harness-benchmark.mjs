...
for (const entry of benchmark.cases) {
  ...
  assert.equal(
    entry.sourceProjects.length,
    entry.sourceUrls.length,
    `${entry.id}: source mismatch`
  );
...
console.log(
  `Priority mix: P0=${priorities.P0?.length ?? 0}, P1=${priorities.P1?.length ?? 0}, P2=${priorities.P2?.length ?? 0}`
);
...
for (const run of efficiency.runs) {
  assert.ok(
    ...
    `${run.agent}: invalid efficiency status`
  );
  ...
  if (run.status === 'passed')
    assert.equal(run.testsPassed, true, `${run.agent}: passed without tests`);
}
console.log(
