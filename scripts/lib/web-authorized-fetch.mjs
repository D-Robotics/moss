export function createAuthorizedWebFetch(fetchImplementation = globalThis.fetch) {
  const csrfByOrigin = new Map();
  return async function authorizedWebFetch(input, init = {}) {
    const url =
    ...
    if (!csrfToken) {
      const bootstrap = await fetchImplementation(`${url.origin}/api/bootstrap`).then((response) =>
        response.json()
      );
